...
def run_simulation(symbol, df, h1_map, h4_map):
    ...
    for i in range(50, len(df) - 10):
        ...
        # VETOES
        if symbol == "GOLD":
            if t.weekday() == 4: continue # Friday
            if not (13 <= hour <= 20): continue # NY Session
        elif symbol == "USDJPY":
            if 12 <= hour <= 21: continue
        elif symbol == "AUDUSD":
            if 8 < hour < 22: continue
        else:
            if not (7 <= hour <= 18) and symbol in ["EURUSD", "GBPUSD"]: continue
            if not (12 <= hour <= 20) and symbol in ["USDCAD", "BTCUSD", "ETHUSD"]: continue
        ...
        if symbol == "GOLD":
            ...
            elif row['RSI'] < 25 and row['close'] < row['vwap'] - row['ATR']: raw_signal = "BUY"
            elif row['RSI'] > 75 and row['close'] > row['vwap'] + row['ATR']: raw_signal = "SELL"
        elif symbol == "USDJPY":
            if macro_trend == 'BULLISH' and row['low'] < row['EMA_50'] and row['close'] > row['EMA_50']: raw_signal = "BUY"
            elif macro_trend == 'BEARISH' and row['high'] > row['EMA_50'] and row['close'] < row['EMA_50']: raw_signal = "SELL"
        elif symbol == "AUDUSD":
...
def main():
    ...
    print("=== FINAL DNA MODULE BACKTEST ===")

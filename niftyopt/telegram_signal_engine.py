...
def extract_via_regex(text: str):
    ...
    if "BUY " in text_upper or "ENTER:" in text_upper or "ENTRY:" in text_upper or "BUY" in text_upper or ("ABOVE" in text_upper and "TG" in text_upper and "SL" in text_upper):
        try:
            ...
            if explicit_enter:
                ...
                if inst_match:
                    ...
                    # Fallback to the first line if it looks like an instrument
                    first_line = text_upper.split('\n')[0].strip()
                    if "CE" in first_line or "PE" in first_line or "CALL" in first_line or "PUT" in first_line:
                        data["instrument"] = first_line.replace("BUY", "").strip()
                    else:
                        return None
            ...
            # Extract first target from potentially multiple targets
...
def save_to_excel(channel_id: int, extracted_data: dict, raw_text: str):
    if extracted_data.get("status") in ["IGNORE", "ERROR"]:
        extracted_data = {
            ...
            "target": "",
            "pnl": 0,
            "parser_used": extracted_data.get("status")
        }

...
print(f"  |  Total PnL                       Rs.{total_pnl:>10,.0f}                                  |")
print(f"  |  Average per Trade               Rs.{avg_trade:>10,.0f}                                  |")
print(f"  |  Average per Day                 Rs.{avg_daily:>10,.0f}                                  |")
print(f"  |  Daily Return %                  {(avg_daily/400000*100):>6.2f}% (on Rs.4L capital)                   |")
print("""  +---------------------------------------------------------------------------+

  +---------------------------------------------------------------------------+
  |  RISK METRICS                                                             |
  +---------------------------------------------------------------------------+""")
print(f"  |  Max Drawdown                    Rs.{max_drawdown:>10,.0f}                                |")
print(f"  |  Green Days                      {green_days}/{total_days} ({green_days/total_days*100:.1f}%)                            |")
print(f"  |  Red Days                        {red_days}/{total_days} ({red_days/total_days*100:.1f}%)                             |")
print(f"  |  Best Day                        Rs.{max_daily_profit:>10,.0f}                                |")
print(f"  |  Worst Day                       Rs.{max_daily_loss:>10,.0f}                                |")
print("""  +---------------------------------------------------------------------------+
...
for i, (date, pnl) in enumerate(daily_sorted.head(10).items(), 1):
    ...
    print(f"  {i:>2}. {date}  Rs.{pnl:>10,.0f}  {trade_str}")
...
for i, (date, pnl) in enumerate(daily_sorted.tail(10).items(), 1):
    ...
    print(f"  {i:>2}. {date}  Rs.{pnl:>10,.0f}  Exits: {reason_str}")
...
for month, pnl in monthly.items():
    bar_len = min(50, max(0, int(abs(pnl) / 1000)))
    bar = "#" * bar_len
    print(f"  {month}  Rs.{pnl:>10,.0f}  ({pnl/400000*100:>+6.1f}%)  {bar}")
...
================================================================================
  ...
  - 33 active strategies (disabled 2 losers: TREND_FOLLOWING, SHORT_UNWIND)
  - Expected: 0.39% daily (Rs.{avg_daily:,.0f}) with Rs.{abs(max_drawdown):,.0f} drawdown

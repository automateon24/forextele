...
# Define execute_tsl_idx with Index-Selective Stagnancy Cut
def execute_tsl_idx_v15_selective_stagnant(entry_bar: pd.Series, remaining: pd.DataFrame, hard_exit: int = 1430, 
    ...
    elapsed_mins = 0
    # Enable stagnancy cut ONLY for BANKNIFTY and FINNIFTY
    use_stagnant_cut = index_name in ['BANKNIFTY', 'FINNIFTY']
    stagnant_mins = 90
    ...
    for _, bar in remaining.iterrows():
        ...
        # 4. INDEX-SELECTIVE THETA-PRESERVATION STAGNANCY CUT
        if use_stagnant_cut and elapsed_mins >= stagnant_mins:
            if close < ep * 1.01:
                xp = close; xr = 'STAGNANT'; xt = ts; break
...
bt.execute_tsl_idx = execute_tsl_idx_v15_selective_stagnant
...
def main():
    print("=" * 70)
    print("RUNNING V15 INDEX-SELECTIVE STAGNANCY CUT BACKTEST")
    print("=" * 70)

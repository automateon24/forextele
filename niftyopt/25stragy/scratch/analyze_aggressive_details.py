...
def analyze():
    csv_path = r'C:\25stragy\backtest_results\strict_aggressive_100k_trades.csv'
    ...
    print("="*80)
    print("AGGRESSIVE 100K PER INDEX PERFORMANCE SUMMARY:")
    ...
    print("\n" + "="*80)
    print("AGGRESSIVE 100K MONTHLY PnL BREAKDOWN PER INDEX:")
    monthly_idx = df.groupby(['month', 'index'])['pnl_rs'].sum().unstack(fill_value=0)
    print(monthly_idx.to_string())

    print("\n" + "="*80)
    print("AGGRESSIVE 100K DAILY CAPITAL UTILIZATION STATS:")
    ...
    print(f"\n  Combined Portfolio Max Drawdown (Aggressive 100k): Rs. {portfolio_drawdown:,.2f}")

    # Daily PnL distribution
    print("\n" + "="*80)
    print("DAILY PnL RETURN DISTRIBUTION:")
    days_pnl = df.groupby('date')['pnl_rs'].sum()
    print(f"  Total Active Trading Days         : {len(days_pnl)}")
    print(f"  Positive (Green) Days             : {len(days_pnl[days_pnl > 0])} ({100*len(days_pnl[days_pnl > 0])/len(days_pnl):.1f}%)")
    print(f"  Negative (Red) Days               : {len(days_pnl[days_pnl < 0])} ({100*len(days_pnl[days_pnl < 0])/len(days_pnl):.1f}%)")
    print(f"  Days making > 5% (Rs. 20,000+)    : {len(days_pnl[days_pnl >= 20000])}")
    print(f"  Days making > 10% (Rs. 40,000+)   : {len(days_pnl[days_pnl >= 40000])}")
    print(f"  Days making > 15% (Rs. 60,000+)   : {len(days_pnl[days_pnl >= 60000])}")
    print(f"  Max Profit in a Single Day        : Rs. {days_pnl.max():,.2f}")
    print(f"  Max Loss in a Single Day          : Rs. {days_pnl.min():,.2f}")

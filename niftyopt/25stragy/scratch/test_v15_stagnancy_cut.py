...
# Define execute_tsl_idx with Stagnancy Cut
def execute_tsl_idx_v15_stagnant(entry_bar: pd.Series, remaining: pd.DataFrame, hard_exit: int = 1430, 
    ...
    elapsed_mins = 0
    # Let's set stagnancy cut threshold: 90 mins, profit threshold: 5%
    stagnant_mins = 90
    profit_thresh = 0.05
    ...
    for _, bar in remaining.iterrows():
        ...
        # 4. THETA-PRESERVATION STAGNANCY CUT
        # If open for stagnant_mins, and profit is less than 5%
        if elapsed_mins >= stagnant_mins:
            if close < ep * (1 + profit_thresh):
...
bt.execute_tsl_idx = execute_tsl_idx_v15_stagnant
...
def main():
    print("=" * 70)
    print("RUNNING V15 THETA-PRESERVATION STAGNANCY CUT BACKTEST")
    print("=" * 70)

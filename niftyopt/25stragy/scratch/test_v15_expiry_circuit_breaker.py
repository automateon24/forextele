...
# Enforce Expiry-Only trading with a daily circuit breaker check in the loop
def run_index_expiry_circuit_breaker(idx_name: str, opt_data: pd.DataFrame, eod_data: pd.DataFrame, cfg: bt.IndexConfig):
    ...
    for day in trading_days:
        ...
        for i in range(3, len(c15)):
            ...
            # Calculate today's PnL across all trades in this index for this day
            today_pnl = sum(t.pnl_rs for t in all_trades if t.date == day)
            if today_pnl <= bt.DAILY_CIRCUIT_BREAKER_RS:
                # print(f"  [{idx_name}] {day} Daily circuit breaker hit (PnL: Rs. {today_pnl:.2f}). Stopping trading.")
                break
...
bt.run_index = run_index_expiry_circuit_breaker

# Active strategies: ONLY ZERO_HERO and GAMMA_BLAST
expiry_strategies = ["ZERO_HERO", "GAMMA_BLAST"]

def run_test_circuit_breaker(capital_per_index: float, max_lots_cap: int, circuit_breaker: float):
    print("\n" + "="*70)
    print(f"RUNNING EXPIRY-ONLY (ZERO_HERO + GAMMA_BLAST)")
    print(f"  Capital per index: Rs. {capital_per_index:,} | Max Lots Cap: {max_lots_cap}")
    print(f"  DAILY CIRCUIT BREAKER: Rs. {circuit_breaker:,}")
    print("="*70)
    
    # Configure in memory
    bt.CAPITAL_PER_INDEX = capital_per_index
    bt.MAX_LOTS_CAP = max_lots_cap
    bt.DAILY_CIRCUIT_BREAKER_RS = circuit_breaker
...
if __name__ == '__main__':
    # Test with Rs. -30,000 daily circuit breaker
    run_test_circuit_breaker(500000, 300, -30000)

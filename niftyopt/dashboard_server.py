...
# --- MASTER CONTROL & HEALTH SYSTEM ---
@app.get("/api/health")
def get_health():
    import json, os
    health_file = r'C:\25stragy\system_health.json'
    if not os.path.exists(health_file):
        default = {"master_control": "ACTIVE", "dhan_api": {"status": "OK", "msg": "Running smoothly"}, "telegram_engine": {"status": "OK", "msg": "Listening"}}
        with open(health_file, 'w') as f: json.dump(default, f)
        return default
    try:
        with open(health_file, 'r') as f:
            return json.load(f)
    except:
        return {"master_control": "ACTIVE", "dhan_api": {"status": "ERROR", "msg": "Cannot read health file"}, "telegram_engine": {"status": "OK", "msg": "Listening"}}

@app.post("/api/control/{action}")
def post_control(action: str):
    import json, os
    health_file = r'C:\25stragy\system_health.json'
    try:
        with open(health_file, 'r') as f: h_data = json.load(f)
    except:
        h_data = {"master_control": "ACTIVE", "dhan_api": {"status": "OK", "msg": ""}, "telegram_engine": {"status": "OK", "msg": ""}}
    
    if action == "TOGGLE":
        h_data["master_control"] = "STOP" if h_data.get("master_control") == "ACTIVE" else "ACTIVE"
    elif action == "STOP":
        h_data["master_control"] = "STOP"
    elif action == "START":
        h_data["master_control"] = "ACTIVE"
    elif action == "CLEAR_ALERTS":
        h_data["dhan_api"] = {"status": "OK", "msg": "Cleared by user"}
        h_data["telegram_engine"] = {"status": "OK", "msg": "Cleared by user"}
    elif action == "SHUTDOWN":
        os.system("taskkill /F /IM python.exe /T")
        
    with open(health_file, 'w') as f: json.dump(h_data, f, indent=4)
    return {"status": "success", "master_control": h_data["master_control"]}
...
def load_modular_trades(csv_path: str, fetcher_inst, cfg_db):
    ...
    try:
        ...
        lot_size = int(cfg_db['index_profiles'].get('NIFTY', {}).get('lot_size', 25))
...
def get_dashboard_data():
    ...
    for t in active_v15:
        ...
        if sec_id_str and str(sec_id_str).lower() != 'nan':
            try:
                clean_sec = str(int(float(sec_id_str)))
                fetcher.register_active_option_id(clean_sec)
                t['option_security_id'] = clean_sec  # update in place so it looks it up correctly later
            except ValueError:
                pass
    ...
    lot_size_nifty = int(config_db['index_profiles'].get('NIFTY', {}).get('lot_size', 25))
    for t in active_v4:
    ...
    try:
        if os.path.exists(r"C:\25stragy\telegram_signals.xlsx"):
            ...
            for _, r in tdf.iterrows():
                ...
                opt_name = str(r.get("instrument", "Unknown"))
                if opt_name.upper() == "TRADE TYPE":
                    raw_msg = str(r.get("raw_message", ""))
                    import re
                    match = re.search(r"Enter:\s*[`*]?([^`*\n]+)[`*]?", raw_msg)
                    if match:
                        opt_name = match.group(1).strip()

                status = str(r.get("status", "NEW_SIGNAL"))
                if status == "NEW_SIGNAL":
                    status = "OPEN"
                    
                pnl_val = float(r.get("pnl", 0.0) if not pd.isna(r.get("pnl")) else 0.0)
                cap_utilized = float(r.get("capital_utilized", 0.0) if not pd.isna(r.get("capital_utilized")) else 0.0)
                
                chan_id = str(r.get("channel_id", "VENDOR")).replace(".0", "")
                chan_name = "Premium"
                if "2231238486" in chan_id: chan_name = "MCX Signals"
                elif "2412774015" in chan_id: chan_name = "Equity/BTST"
                elif "2871728862" in chan_id: chan_name = "Zero to Hero"
                elif "2902210804" in chan_id: chan_name = "Sensex360"
                elif "2626583811" in chan_id: chan_name = "BTST VIP+"
                elif "2115753582" in chan_id: chan_name = "Dil Se Trader"
                elif "2264960458" in chan_id: chan_name = "VIP Options"

                ts = str(r.get("timestamp", ""))
                entry_time = ts.split(" ")[1][:5] if " " in ts else ts
                exit_time_raw = str(r.get("exit_time", ""))
                exit_time = exit_time_raw.split(" ")[1][:5] if " " in exit_time_raw else (exit_time_raw if exit_time_raw != "nan" else "-")
                if exit_time == "nan": exit_time = "-"
                ...
                if os.path.exists(live_prices_path):
                    try:
                        with open(live_prices_path, 'r') as f:
                            lp_dict = json.load(f)
                            live_price = float(lp_dict.get(opt_name, 0.0))
                    except:
                        pass
                
                current_target_px = str(r.get("target", "0"))
                if status == "OPEN" and live_price > 0:
                    current_target_px = f"{live_price:.2f}"

                try:
                    ep_str = str(r.get("entry_range", "0")).split('-')[0].strip()
                    ep = float(ep_str)
                except ValueError:
                    ep = 0.0
                    
                unreal_pnl_val = 0.0
                if status == "OPEN" and live_price > 0 and ep > 0:
                    lot_size = 15
                    if "NIFTY" in opt_name.upper() and "BANK" not in opt_name.upper(): lot_size = 25
                    if "SENSEX" in opt_name.upper(): lot_size = 10
                    if "MIDCP" in opt_name.upper(): lot_size = 75
                    if "FINNIFTY" in opt_name.upper(): lot_size = 40
                    unreal_pnl_val = (live_price - ep) * lot_size
                    if str(r.get("action", "BUY")).upper() == "SELL":
                        unreal_pnl_val = -unreal_pnl_val

                if opt_name.upper() == "UNKNOWN" and "Goldm" in raw_msg:
                    opt_name = "GOLDM PUT"
                ...
                trade_obj = {
                    ...
                    "current_price": current_target_px,
                    "cmp": f"{live_price:.2f}" if live_price > 0 else "N/A",
                    "greeks": {"delta": 0, "theta": 0, "iv": 0},
                    "unrealized_pnl": round(unreal_pnl_val, 2),
                    "status": status,
                    "pnl_rs": pnl_val,
                ...
                if status in ["CLOSED_SL", "MANUAL_BOOK", "T3_HIT", "TSL_HIT_AT_COST", "TSL_HIT_AT_T1", "SL_HIT", "EOD_CLOSE", "EOD_CLOSE_UNTRACKED"]:
        ...
        logger.error(f"Error parsing telegram excel: {e}")
            
    # Deduplicate trades to filter out scraper anomalies
    def dedup_trades(trade_list):
        seen = set()
        deduped = []
        for t in trade_list:
            key = (t['option_name'], t['direction'], t['status'], t['entry_price'])
            if key not in seen:
                seen.add(key)
                deduped.append(t)
        return deduped

    telegram_active = dedup_trades(telegram_active)
    telegram_completed = dedup_trades(telegram_completed)

    res_payload = {
        "telegram_connected": telegram_connected,
        "telegram_trades": telegram_active,
        ...
        "telegram": {
            "summary": {
                "capital_base": TELEGRAM_CAPITAL,
                "margin_used": tele_margin,
                "available_capital": max(0.0, TELEGRAM_CAPITAL - tele_margin),
                "realized_pnl": round(tele_realized, 2),
                "unrealized_pnl": round(sum(t.get("unrealized_pnl", 0.0) for t in telegram_active), 2),
                "net_pnl": round(tele_realized + sum(t.get("unrealized_pnl", 0.0) for t in telegram_active), 2)
            },
            "active_trades": telegram_active,
            "completed_trades": telegram_completed
        }
    }
...
def get_dashboard_ui():
    ...
    <body>
        ...
        <!-- 5 Indices Tracker (Global context) -->
        ...
        <!-- Metric Summary Cards (Contextual to Tab) -->
        <div class="metrics-grid">
            <div class="metric-card info">
                <div class="metric-title">Capital Base</div>
                <div class="metric-value" id="capitalBase">Rs. 0.00</div>
                <div class="metric-sub" id="capitalSub">Shared Portfolio Base</div>
            </div>
        ...
        <!-- Terminal / Logs Panel (Moved to bottom) -->
        <div class="panel-card" style="margin-bottom: 24px;">
            ...
            function renderDashboard(data) {
                ...
                if (activeHead) {
                    if (currentEngine === 'telegram') {
                        activeHead.innerHTML = `<tr><th>Channel ID</th><th>Instrument</th><th>Action</th><th>Lots</th><th>Entry Range</th><th>Target / SL</th><th>CMP</th><th>Status</th><th>Unrealized PnL (Rs)</th></tr>`;
                    } else {
                ...
                if (completedHead) {
                    if (currentEngine === 'telegram') {
                        completedHead.innerHTML = `<tr><th>Channel ID</th><th>Instrument</th><th>Action</th><th>Lots</th><th>Entry Range</th><th>Exit State</th><th>Status</th><th>Realized PnL (Rs)</th></tr>`;
                    } else {
                        completedHead.innerHTML = `<tr><th>Index</th><th>Strategy</th><th>Dir</th><th>Lots</th><th>Entry Px</th><th>Exit Px</th><th>Exit Reason</th><th>Realized PnL</th></tr>`;
                    }
                }
                ...
                // Utility for grouping trades
                window.openGroups = window.openGroups || new Set();
                
                function renderGroupedTrades(trades, tableId, engine, isActive) {
                    try {
                        const table = document.getElementById(tableId);
                        if (trades.length === 0) {
                            table.innerHTML = `<tr><td colspan="9" style="text-align: center; color: var(--text-muted); padding: 40px 0;">No ${isActive ? 'active' : 'completed'} trades.</td></tr>`;
                            return;
                        }
                    
                    const groups = {};
                    trades.forEach(t => {
                        const key = engine === 'telegram' ? (t.channel_name || 'Premium') : (t.strategy || 'Unknown');
                    ...
                    const prefix = isActive ? 'act' : 'cmp';
                    
                    for (const [groupName, groupTrades] of Object.entries(groups)) {
                        grpIdx++;
                        const groupId = `${prefix}-grp-${grpIdx}`;
                        
                        let totalLots = 0;
                        let totalPnl = 0;
                        groupTrades.forEach(t => {
                            totalLots += parseInt(t.lots) || 0;
                            totalPnl += (isActive ? parseFloat(t.unrealized_pnl || 0) : parseFloat(t.pnl_rs || 0));
                        });
                        
                        const isOpen = window.openGroups.has(groupId);
                        const displayStyle = isOpen ? 'table-row' : 'none';
                        const toggleIcon = `<span style="cursor:pointer; display:inline-block; width:15px; margin-right:5px; transition: transform 0.2s;" id="icon-${groupId}">${isOpen ? '▼' : '▶'}</span>`;
                        const pnlClass = totalPnl >= 0 ? 'price-up' : 'price-down';
                        
                        // Header row
                        html += `
                            <tr style="background: rgba(255,255,255,0.04); cursor: pointer; border-left: 3px solid var(--accent-blue);" onclick="toggleAccordion('${groupId}')">
                                <td colspan="3"><strong>${toggleIcon} ${groupName}</strong> <span style="font-size:10px; color:var(--text-muted); margin-left:8px;">(${groupTrades.length} positions)</span></td>
                                <td><strong>${totalLots}</strong></td>
                                <td colspan="${engine === 'telegram' ? '4' : '3'}"></td>
                                <td class="pnl-cell ${pnlClass}" style="font-weight:700;">${formatCurrency(totalPnl)}</td>
                            </tr>
                        `;
                        ...
                        if (isActive) {
                            groupTrades.forEach(t => {
                                const childPnlClass = parseFloat(t.unrealized_pnl || 0) >= 0 ? 'price-up' : 'price-down';
                                if (engine === 'telegram') {
                                    html += `
                                        <tr class="${groupId}" style="display: ${displayStyle}; background: rgba(0,0,0,0.2);">
                                            <td style="padding-left:25px;"><strong style="color: var(--accent-blue); font-size: 11px;">${t.index}</strong></td>
                                            <td style="font-family: 'JetBrains Mono', monospace; font-size: 12.5px;">${t.option_name}</td>
                                            <td><span class="badge ${t.direction ? t.direction.toLowerCase() : ''}">${t.direction || ''}</span></td>
                                            <td>${t.lots}</td>
                                            <td><span style="color: var(--accent-yellow);">${t.entry_price}</span><br><span style="font-size: 10px; color: var(--text-muted);">${t.entry_time || 'N/A'}</span></td>
                                            <td><strong style="color: var(--accent-green);">${t.current_price}</strong><br><span style="font-size: 10px; color: var(--text-muted);">${t.exit_time || '-'}</span></td>
                                            <td><strong style="color: var(--accent-blue);">${t.cmp || 'N/A'}</strong></td>
                                            <td><span class="badge" style="background: rgba(245, 158, 11, 0.1); color: var(--accent-yellow); font-size: 10px;">${t.status}</span></td>
                                            <td class="pnl-cell ${childPnlClass}">${formatCurrency(t.unrealized_pnl || 0)}</td>
                                        </tr>
                                    `;
                                } else {
                                    html += `
                                        <tr class="${groupId}" style="display: ${displayStyle}; background: rgba(0,0,0,0.2);">
                                            <td style="padding-left:25px;"><strong style="color: var(--accent-blue);">${t.index}</strong></td>
                                            <td style="font-family: 'JetBrains Mono', monospace; font-size: 12.5px;">${t.option_name}</td>
                                            <td><span class="badge ${t.direction ? t.direction.toLowerCase() : ''}">${t.direction || ''}</span></td>
                                            <td>${t.lots}</td>
                                            <td>${formatCurrency(t.entry_price)}<br><span style="font-size: 10px; color: var(--text-muted);">${t.entry_time || 'N/A'}</span></td>
                                            <td><strong>${formatCurrency(t.current_price)}</strong><br><span style="font-size: 10px; color: var(--text-muted);">${t.exit_time || '-'}</span></td>
                                            <td>
                                                <div class="greeks-display">
                                                    <span>&Delta;: <span class="greek-val">${t.greeks ? t.greeks.delta : '0.0'}</span></span>
                                                    <span>&theta;: <span class="greek-val">${t.greeks ? t.greeks.theta : '0.0'}</span></span>
                                                    <span>IV: <span class="greek-val">${t.greeks ? t.greeks.iv : '0'}%</span></span>
                                                </div>
                                            </td>
                                            <td class="pnl-cell ${childPnlClass}">${formatCurrency(t.unrealized_pnl || 0)}</td>
                                        </tr>
                            ...
                            groupTrades.slice().reverse().forEach(t => {
                                const childPnlClass = parseFloat(t.pnl_rs || 0) >= 0 ? 'price-up' : 'price-down';
                                if (engine === 'telegram') {
                                    html += `
                                        <tr class="${groupId}" style="display: ${displayStyle}; background: rgba(0,0,0,0.2);">
                                            <td style="padding-left:25px;"><strong style="color: var(--accent-blue); font-size: 11px;">${t.index}</strong></td>
                                            ...
                                            <td><span class="badge ${t.direction ? t.direction.toLowerCase() : ''}">${t.direction || ''}</span></td>
                                            <td>${t.lots}</td>
                                            <td>${t.entry_price}<br><span style="font-size: 10px; color: var(--text-muted);">${t.entry_time || 'N/A'}</span></td>
                                            <td><strong style="color: var(--accent-green);">${t.current_price || '-'}</strong><br><span style="font-size: 10px; color: var(--text-muted);">${t.exit_time || t.timestamp || '-'}</span></td>
                                            <td><span style="font-size: 12px; font-weight: 500; color: var(--accent-yellow);">${t.exit_reason || 'BOOKED'}</span></td>
                                            <td class="pnl-cell ${childPnlClass}">${formatCurrency(t.pnl_rs || 0)}</td>
                                        </tr>
                                    `;
                                } else {
                                    html += `
                                        <tr class="${groupId}" style="display: ${displayStyle}; background: rgba(0,0,0,0.2);">
                                            <td style="padding-left:25px;"><strong>${t.index}</strong></td>
                                            <td style="font-size: 12.5px; color: var(--text-muted);">${t.strategy}</td>
                                            <td><span class="badge ${t.direction ? t.direction.toLowerCase() : ''}">${t.direction}</span></td>
                                            <td>${t.lots}</td>
                                            <td>${formatCurrency(t.entry_price)}<br><span style="font-size: 10px; color: var(--text-muted);">${t.entry_time || 'N/A'}</span></td>
                                            <td>${formatCurrency(t.exit_price)}<br><span style="font-size: 10px; color: var(--text-muted);">${t.exit_time || '-'}</span></td>
                        ...
                        console.error('Error in renderGroupedTrades:', err);
                    }
                }
                
                renderGroupedTrades(activeTrades, 'activeTradesTable', currentEngine, true);
                renderGroupedTrades(completedTrades, 'completedTradesTable', currentEngine, false);
                
                
                window.toggleAccordion = function(groupId) {
                    if (window.openGroups.has(groupId)) {
                        window.openGroups.delete(groupId);
                    } else {
                        window.openGroups.add(groupId);
                    }
                    
                    const rows = document.querySelectorAll('.' + groupId);
                    const icon = document.getElementById('icon-' + groupId);
                    const isNowOpen = window.openGroups.has(groupId);
                    
                    rows.forEach(r => {
                        r.style.display = isNowOpen ? 'table-row' : 'none';
                    });
                    
                    if (icon) {
                        icon.innerText = isNowOpen ? '▼' : '▶';
                    }
                };

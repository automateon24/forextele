...
def scan_dna():
    ...
    for sym in symbols:
        for tf_name, tf_code in timeframes.items():
            df_bars = fetch_bars(sym, tf_code, 3000)
            ...
            strategies = [
                ...
                ("ASIAN_SCALP", AsianRangeScalpStrategy(sym)),
                ("BOLLINGER_MR", BollingerMeanReversionStrategy(sym)),
            ...
            for s_name, strat in strategies:
                # To test MTF Alignment impact, we just run the engine.
                # Right now engine applies MTF to Trend Momentum, SMC_ORDER_BLOCK, FVG_RETEST, CHART_PATTERN_SWING
                try:
    ...
    print("\n" + "="*120)
    print("  ST3 DNA DISCOVERY MATRIX (Capital: $1500.0)")
    print("="*120)
    print(f"  {'Symbol':<10} {'TF':<5} {'Strategy':<25} {'Trades':<8} {'Win %':<8} {'Net PnL':<12} {'ROI %':<8} {'PF':<6} {'RR':<6} {'Max DD %':<10} {'Status'}")
    print("  " + "-"*115)

    for _, r in res_df.iterrows():
        print(f"  {r['symbol']:<10} {r['tf']:<5} {r['strategy']:<25} {r['trades']:<8} {r['win_rate']:>5.1f}%   ${r['net_pnl']:>9.2f}  {r['roi']:>6.1f}%  {r['pf']:>5.2f}  {r['rr']:>4.2f}  {r['dd']:>6.1f}%    {r['status']}")

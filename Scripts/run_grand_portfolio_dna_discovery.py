"""
Grand Portfolio Trading DNA Discovery Engine (All 8 Pairs × All 15 Strategies × H1, M15, M5)
==========================================================================================
1. Evaluates ALL 15 strategies across ALL 8 Major Trading Pairs:
   GOLD, EURUSD, GBPUSD, USDJPY, AUDUSD, USDCAD, USDCHF, NZDUSD
2. Evaluates across H1, M15, M5 timeframes (360 total permutations).
3. Captures Zonal / Session features (Asian, London, NY sessions, Session overlap).
4. Live-Realistic Execution Engine:
   Pessimistic same-bar SL, gap fills, symbol-specific spreads & slippage, symbol-aware TSL, risk gates.
5. Strict 70/30 Out-Of-Sample (OOS) Walk-Forward ML validation.
6. Saves surviving ML models (.pkl files) to models/ for live deployment.
"""
...
logger = logging.getLogger("PORTFOLIO_DNA")
...
SYMBOL_PARAMS = {
    ...
    "AUDUSD": {"spread_points": 1.00, "slippage_usd": 0.00005},
    "USDCAD": {"spread_points": 1.20, "slippage_usd": 0.00006},
    "USDCHF": {"spread_points": 1.20, "slippage_usd": 0.00006},
    "NZDUSD": {"spread_points": 1.50, "slippage_usd": 0.00006},
}
...
def run_oos_backtest(test_df: pd.DataFrame, strategies, cost_model, capital: float, volume: float, models: dict, symbol: str, use_ml: bool = False) -> pd.DataFrame:
    ...
    is_gold = "GOLD" in symbol or "XAU" in symbol
...
def main():
    ...
    print("\n" + "="*100)
    print("  GRAND PORTFOLIO TRADING DNA DISCOVERY ENGINE")
    print("  Symbols: 8 Major Pairs (GOLD, EURUSD, GBPUSD, USDJPY, AUDUSD, USDCAD, USDCHF, NZDUSD)")
    print("  Timeframes: H1, M15, M5 | All 15 Codebase Strategies")
    ...
    for symbol in SYMBOLS:
        ...
        for tf in TIMEFRAMES:
            logger.info(f"Analyzing Trading DNA for {symbol} [{tf}]...")
            try:
                df = fetch_bars(symbol, tf, BARS_TO_FETCH)
                df = precompute_df_features(df)
            ...
            for strat in active_strategies:
                ...
                if model is not None:
                    models[s_id] = model
                    model_file = models_dir / f"ml_{symbol}_{s_id}_{tf}.pkl"
    ...
    report_df = pd.DataFrame(all_dna_results)
    report_path = reports_dir / f"grand_portfolio_dna_discovery_{datetime.now().strftime('%Y%m%d_%H%M')}.csv"
    report_df.to_csv(report_path, index=False)

    print("\n" + "="*100)
    print("  GRAND PORTFOLIO TRADING DNA DISCOVERY — SURVIVING STRATEGIES (UNSEEN HOLDOUT)")
    print("="*100)

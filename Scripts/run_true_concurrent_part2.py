...
logger = logging.getLogger("TRUE_CONCURRENT_PART2")
...
def main():
    ...
    for sym in symbols:
        ...
        strat_sweep = LiquiditySweepStrategy(symbol=sym)
        strat_ob = SMCOrderBlockStrategy(symbol=sym)
        ...
        if "GOLD" in sym:
            ...
            cost_m = CostModel(spread_points=0.015)
        else:
            cost_m = CostModel(spread_points=0.00015)
            
        # We need an engine for M15 execution
        ...
        asset_data[sym] = {
            ...
            "h1": df_h1,
            "strats": {
                "LIQUIDITY_SWEEP_M15": strat_sweep,
                "SMC_ORDER_BLOCK_M15": strat_ob
            },
            "engine": engine_m15
        }
    ...
    logger.info(f"Starting true concurrent Part 2 timeline at {start_time}")
    ...
    for i in range(3000, len(master_timeline)):
        ...
        # Session Filter applied dynamically
        if not is_prime_trading_hour(current_time):
            continue
            
        signals_to_process = []
        
        # 2. Check Strategies for ALL symbols
        if current_time.minute % 15 == 0:
            for sym, data in asset_data.items():
                ...
                for strat_id, strat in data["strats"].items():
                    ...
                    if sig:
                        # MTF Validation (Only take trades aligned with H1)
        ...
        for sig, strat_id, engine, window in signals_to_process:
            # Enforce 1 trade per (symbol, strategy)
    ...
    logger.info("=== TRUE CONCURRENT PART 2 COMPLETE ===")

...
def get_macro_trends(symbol):
    # Get H1 and H4 trends for confidence scoring
...
def backtest_advanced_paths(df, h1_map, h4_map, symbol):
    trades = []
    golden_count = 0
    standard_count = 0
    ...
    for i in range(50, len(df) - 10):
        ...
        # SMC Filter
        smc_approved = False
        if raw_signal == "BUY" and h1_trend == 'BULLISH':
            if row['close'] > df.iloc[i]['last_sl']: smc_approved = True
        elif raw_signal == "SELL" and h1_trend == 'BEARISH':
            if row['close'] < df.iloc[i]['last_sh']: smc_approved = True
        ...
        # PATH 1: Dynamic Position Sizing
        is_golden = False
        current_risk_pct = base_risk
        if h1_trend == h4_trend:
            current_risk_pct = base_risk * 2.5 # 2.5% risk for Golden Setup
            is_golden = True
    ...
    return len(trades), golden_count, standard_count, net_profit

def run_detailed_path1(symbols, bars):
    results = []
    total_profit = 0
    for sym in symbols:
        ...
        total_trades, golden, standard, profit = backtest_advanced_paths(df, h1_map, h4_map, sym)
        total_profit += profit
        
        results.append({
            "Symbol": sym,
            "Total Trades": total_trades,
            "Golden Setups (2.5% Risk)": golden,
            "Standard Setups (1% Risk)": standard,
            "Path 1 Net Profit": f"${profit:.2f}"
        })
        
    res_df = pd.DataFrame(results)
    print("=== PATH 1: DYNAMIC AI CONFIDENCE SIZING (PAIR BREAKDOWN) ===")
    print(res_df.to_string(index=False))
    print(f"\n[PORTFOLIO] Path 1 Total Net Profit: ${total_profit:.2f}")
...
def main():
    ...
    run_detailed_path1(symbols, bars)
    mt5.shutdown()

"""
backtest_high_res.py
=======================
Loads 50,000 bars of M1/M5/M15/H1 data and runs all 41 strategies properly.
Uses proper M1 simulation to avoid false SL hits and restores Scalping on Gold/Silver.
"""
import json
import logging
...
DIGITS  = {
    "EURUSD":5, "GBPUSD":5, "USDJPY":3, "AUDUSD":5,
    "GOLD":2, "SILVER":3, "BTCUSD":2, "ETHUSD":3,
}

# The absolute worst performing strategies across 1 year of testing are disabled here to protect capital
DISABLED_STRATEGIES = {"TREND_FOLLOWING", "MAGIC_SQUARE", "EARLY_BREAKDOWN", "MOMENTUM_BURST", "LONG_LIQUIDATION", "BREAKOUT", "BOLLINGER_SQUEEZE", "DAY_HIGH_BEARISH"}
...
def load_tf(symbol, tf):
    p = BASE_DIR / f"data_highres_{symbol}_{tf}.parquet"
...
def generate_signals(symbol, df15, df5, df1h_ended, dna_db):
    ...
    for strat_key, dna in symbol_dnas.items():
        ...
        # Force M5 dataframe for scalping explicitly to ensure high resolution
...
def main():
    ...
    for symbol in SYMBOLS:
        ...
        if df_m15 is None or df_m5 is None or df_m1 is None:
            log.warning("Missing high res data for %s — skipping", symbol)
            continue

        log.info("  Generating indicators...")
        ...
        point = POINT.get(symbol, 0.00001)
        results = simulate_outcomes(signals, df_m1, point)
        log.info("  %d outcomes simulated", len(results))
        all_results.extend(results)

    if not all_results:
        log.error("No results. Run fetch_high_res_data.py first.")
        return

...
from intraday_q_learning import IntradayQBrain

# ─────────────────────────────────────────────────────────────────────────────
# AI/ML Micro-Tuner Instance
# ─────────────────────────────────────────────────────────────────────────────
q_brain = IntradayQBrain()
...
def update_active_trades_exits(now_time: datetime):
    ...
    for trade in list(active_trades):
        ...
        # Update highest premium seen and persist to survive restarts
        if current_ltp > trade.highest_premium:
            trade.highest_premium = current_ltp
            # Only save if it's a meaningful jump to avoid excessive I/O (e.g. > 1%)
            if not hasattr(trade, '_last_saved_premium') or current_ltp > trade._last_saved_premium * 1.01:
                trade._last_saved_premium = current_ltp
                save_trade_state_to_csv()
        ...
        if current_hhmm >= hard_exit_time:
            ...
            # Dynamic TSL Trail based on how far in profit we are
            profit_pct = (trade.highest_premium / trade.entry_price) - 1.0
            
            # If we are up more than 50%, tighten the trail to lock in the massive gains
            active_trail = tsl_trail * 0.5 if profit_pct > 0.50 else tsl_trail
            active_trail = max(active_trail, 0.02) # never tighter than 2%
            
            tsl_floor = trade.highest_premium * (1 - active_trail)
            
            # Lock in at least cost if we've activated TSL
            tsl_floor = max(tsl_floor, trade.entry_price * 1.01)
        ...
        if exit_triggered:
            ...
            # Register with Q-Brain for on-the-fly learning
            current_hhmm = now_time.hour * 100 + now_time.minute
            q_brain.register_closed_trade(trade.strategy, trade.index, trade.pnl_rs, current_hhmm)
        ...
        try:
            ...
            for strat in STRATEGY_DEFS:
                ...
                for direction in directions:
                    ...
                    # Q-Brain ML adjustments
                    q_strat_mult = q_brain.get_strategy_multiplier(strat.name, current_hhmm)
                    q_idx_mult = q_brain.get_index_multiplier(idx_name)
                    
                    if q_strat_mult == 0.0 or q_idx_mult == 0.0:
                        logger.warning(f"  {idx_name} {strat.name} ({direction}): Triggered but skipped (BLOCKED by IntradayQBrain ML).")
                        continue
                        
                    # Deploy cap for this trade is scaled based on CAPITAL_PER_INDEX and Q-Brain
                    deploy_cap = CAPITAL_PER_INDEX * get_tier_deploy_pct(strat.name) * q_strat_mult * q_idx_mult
...
def run_telegram_signal_scan(now_time: datetime):
    """Scan the Telegram signals excel and convert NEW_SIGNAL into active AI-managed trades."""
    global active_trades
    tele_path = r"C:\25stragy\telegram_signals.xlsx"
    ...
    try:
        ...
        for idx, row in df.iterrows():
            ...
            if status == 'NEW_SIGNAL' and ts.startswith(today_str):
                inst = str(row.get('instrument', ''))
                
                # Parse NIFTY 50 24250 CE
                idx_name = 'NIFTY'
                if 'BANK' in inst.upper(): idx_name = 'BANKNIFTY'
                elif 'FIN' in inst.upper(): idx_name = 'FINNIFTY'
                elif 'MIDCP' in inst.upper(): idx_name = 'MIDCPNIFTY'
                elif 'SENSEX' in inst.upper(): idx_name = 'SENSEX'
                
                dir_match = re.search(r'(CE|PE|CALL|PUT)', inst.upper())
                direction = 'CE'
                if dir_match and dir_match.group(1) in ['PE', 'PUT']:
                    direction = 'PE'
                    
                strike_match = re.search(r'(\d{5})', inst)
                if not strike_match:
                    continue
                num_strike = float(strike_match.group(1))
                
                # Try to get option security ID from cache
                chain = option_chains_cache.get(idx_name, {})
                contract = chain.get(num_strike, {}).get(direction)
                if not contract:
                    continue
                ...
                try:
                    entry_range = str(row.get('entry_range', '0'))
                    entry_px = float(re.search(r'[\d\.]+', entry_range).group())
                except:
                    continue
                    
                spot = index_states[idx_name].spot
                if spot <= 0: spot = entry_px * 100 # fallback
                
                # Fixed 1 lot for Telegram as requested
                actual_lots = 1
                
                spot_sl_level = spot * 0.98 if direction == 'CE' else spot * 1.02
                ...
                # Mark as tracked
                df.at[idx, 'status'] = 'TRACKED_BY_AI'
                updated = True
                logger.info(f"[TELEGRAM ENTRY] Took over signal: {inst} | Strike: {num_strike} | Premium: Rs. {entry_px}")
                
        if updated:
            df.to_excel(tele_path, index=False)
    except Exception as e:
        logger.error(f"Error scanning Telegram signals: {e}")
...
def get_tele_sec_id(inst_str: str):
    ...
    elif 'MID' in inst: sec_id = 442
    elif 'SENSEX' in inst: sec_id = 51
    ...
    exch_seg = 'IDX_B' if sec_id == 51 else 'IDX_I'
    
    try:
        exp_r = _api_call(dhan_manager.client.expiry_list, under_security_id=sec_id, under_exchange_segment=exch_seg)
        if not exp_r or exp_r.get('status') != 'success': return None
        expiries = exp_r.get('data', {}).get('data', [])
        if not expiries: return None
        
        oc = _api_call(dhan_manager.client.option_chain, under_security_id=sec_id, under_exchange_segment=exch_seg, expiry=expiries[0])
    ...
    try:
        ...
        for idx in active_idx:
            ...
            if cache_obj:
                ...
                if cache_obj['idx'] == 51: bse_fno.append(int(cache_obj['id']))
        ...
        for idx in active_idx:
            if idx not in tracking:
                if now_time.time() >= datetime.strptime("15:15:00", "%H:%M:%S").time():
                    df.at[idx, 'status'] = "EOD_CLOSE_UNTRACKED"
                    df.at[idx, 'exit_time'] = now_time.strftime("%Y-%m-%d %H:%M:%S")
                    try:
                        entry = float(re.search(r'[\d\.]+', str(df.loc[idx, 'entry_range'])).group())
                        df.at[idx, 'pnl'] = 0  # 0 pnl since untracked
                    except:
                        pass
                    changed = True
                continue
            ...
            if 'highest_premium' not in df.columns:
                df['highest_premium'] = 0.0
            
            existing_high = float(row.get('highest_premium', 0.0) or 0.0)
            if existing_high > 0:
                c_obj['highest'] = max(c_obj['highest'], existing_high)
                
            c_obj['highest'] = max(c_obj['highest'], current_ltp)
            highest = c_obj['highest']
            
            df.at[idx, 'highest_premium'] = highest
            ...
            if not exit_triggered and now_time.time() >= datetime.strptime("15:15:00", "%H:%M:%S").time():
                exit_triggered = True; reason = "EOD_CLOSE"; exit_px = current_ltp
                
            if exit_triggered:
                df.at[idx, 'status'] = reason
                lot_size = 25
                if 'BANK' in inst_str: lot_size = 15
                elif 'FIN' in inst_str: lot_size = 25
                elif 'MID' in inst_str: lot_size = 50
                elif 'SENSEX' in inst_str: lot_size = 10
                df.at[idx, 'pnl'] = int(round((exit_px - entry) * lot_size))
                if df['exit_time'].dtype != 'object': df['exit_time'] = df['exit_time'].astype('object')
                df.at[idx, 'exit_time'] = now_time.strftime("%Y-%m-%d %H:%M:%S")
                changed = True
    ...
    while True:
        try:
            ...
            if current_time_num >= 1530:
                ...
                # Ensure telegram trades are processed before exiting

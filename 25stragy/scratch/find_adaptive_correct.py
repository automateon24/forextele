...
# Copy V15 engine (already copied but let's be sure)
try:
    ...
    print("Error copying engine_v15.py:", e)
...
for sp in search_paths:
    ...
    for root, dirs, files in os.walk(sp):
        # Exclude trading_env, venv, backups, etc.
        if any(x in root.lower() for x in ["venv", "trading_env", "backups", "scratch", "node_modules", ".git", "__pycache__"]):
            continue
        for f in files:

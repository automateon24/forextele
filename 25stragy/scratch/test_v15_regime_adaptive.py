...
# Define the execute_tsl_idx with Regime-Adaptive TSL Capping
def execute_tsl_idx_v15_adaptive(entry_bar: pd.Series, remaining: pd.DataFrame, hard_exit: int = 1430, 
    ...
    # Regime-Adaptive Capping
    is_trending_day = 'TRENDING' in regime or regime == 'EXPLOSIVE_GAP'
    
    if is_trending_day:
        # Relax caps on trending/explosive days to give trades holding power
        tsl_activate = min(0.12, dna.tsl_activate)
        tsl_trail = min(0.08, dna.tsl_trail)
    else:
        # Tight caps on choppy/normal days to guard against option decay
    ...
    # Apply standard regime multipliers (applied after capping in standard engine logic)
...
bt.execute_tsl_idx = execute_tsl_idx_v15_adaptive
...
def main():
    print("=" * 70)
    print("RUNNING V15 REGIME-ADAPTIVE TSL CAPPING BACKTEST")
    print("=" * 70)

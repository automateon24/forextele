...
# Implement the Ultimate Unified Engine logic in run_index with Strict Capital Gating
def run_index_strict_capital(idx_name: str, opt_data: pd.DataFrame, eod_data: pd.DataFrame, cfg: bt.IndexConfig):
    base_strats = bt.make_strategies_v8()
    
    # Load all active strategies from production config
    ...
    for day in trading_days:
        ...
        for i in range(3, len(c15)):
            ...
            for t in all_trades:
                if t.date == day:
                    # If trade's entry_time <= ts < exit_time, it is active
            ...
            for strat in active_strats:
                ...
                for direction in dirs:
                    ...
                    # Sizing based on available capital
...
bt.run_index = run_index_strict_capital

def run_test_strict():
    print("\n" + "="*70)
    print("RUNNING STRICT CAPITAL GATED ULTIMATE ENGINE")
    print("  Capital per index: Rs. 150,000 (Hard Concurrent Cap) | Max Lots Cap: 60")
    print("="*70)
    
    # Configure in memory
    bt.CAPITAL_PER_INDEX = 150000
    bt.MAX_LOTS_CAP = 60
    bt.DAILY_CIRCUIT_BREAKER_RS = -9000
    ...
    if rows:
        ...
        out.to_csv(r'C:\25stragy\backtest_results\strict_trades.csv', index=False)
        print(f"\n  Saved {len(out)} trades to C:\\25stragy\\backtest_results\\strict_trades.csv")

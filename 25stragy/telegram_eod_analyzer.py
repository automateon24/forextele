...
DAILY_REPORT_PATH = r"C:\25stragy\telegram_daily_pnl.md"
AUDIT_REPORT_PATH = r"C:\25stragy\telegram_eod_audit.json"
...
def run_daily_telegram_analysis():
    ...
    try:
        ...
        prompt = f"Here is today's Telegram PnL data: {json.dumps(daily_audit['channels'])}. Give a complete Post-Mortem Analysis including:\n1. What pattern/setup was matching for the winners.\n2. Why were trades profitable vs why were they lost (market chop, time decay, false breakouts).\n3. Which channel performed the best and the worst.\n4. Add a specific AI Learning Timestamp."
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[{"role": "user", "content": prompt}]
        )
        insight = response.choices[0].message.content.strip()
        print(f"\n[AI LEARNING TIMESTAMP: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}]\n🧠 AI Insight:\n{insight}")
        
        # Append AI insight to the markdown report
        with open(DAILY_REPORT_PATH, 'a', encoding='utf-8') as f:
            f.write(f"\n\n## 🧠 AI Behavioral Pattern Analysis\n*Learning Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*\n\n{insight}\n")

...
INDEX_CONFIGS: Dict[str, IndexConfig] = {
    ...
    'SENSEX':     IndexConfig('SENSEX',     10,  100, 4, premium_scale=4.0,  hard_exit=1430, max_ce_day=2, wide_range_pts=350,  entry_cutoff=1430),
}
...
def execute_tsl_idx(entry_bar: pd.Series, remaining: pd.DataFrame, hard_exit: int = HARD_EXIT, 
    ...
    # Cap parameters for option safety
    tsl_activate = min(0.06, dna.tsl_activate)
    tsl_trail = min(0.04, dna.tsl_trail)
    target_pct = dna.target_pct
    sl_backstop = dna.sl_backstop
    
    # Scale by index multipliers
    multipliers = INDEX_TSL_MULTIPLIERS.get(index_name, {'activate': 1.0, 'trail': 1.0, 'target': 1.0})
    tsl_activate *= multipliers['activate']
    tsl_trail *= multipliers['trail']
        ...
        for i in range(3, len(c15)):
            ...
            for strat in active_strats:
                ...
                for direction in dirs:
                    ...
                    # Dynamic Lot Sizing based on Capital (scaled by CAPITAL / 100,000)
                    actual_lots = get_base_lots(strat.name) * (CAPITAL // 100_000)

...
# Force EXPIRY_UNCAP_TIGHT to True in memory
bt.EXPIRY_UNCAP_TIGHT = True
if "EXPIRY_UNCAP_TIGHT" in os.environ:
    del os.environ["EXPIRY_UNCAP_TIGHT"]

# Define execute_tsl_idx with Spot-Based Stop Loss and correct expiry handling
def execute_tsl_idx_v15_spot_sl_correct(entry_bar: pd.Series, remaining: pd.DataFrame, hard_exit: int = 1430, 
    ...
    # Uncapped expiry trails for ZERO_HERO and GAMMA_BLAST on expiry days (respecting EXPIRY_UNCAP_TIGHT)
    if bt.ENABLE_EXPIRY_UNCAP:
        if is_expiry and (strat_name in ['ZERO_HERO', 'GAMMA_BLAST']):
            target_pct = 999.0
            if not bt.EXPIRY_UNCAP_TIGHT:
                tsl_activate = 0.25
                tsl_trail = 0.15
    ...
    for _, bar in remaining.iterrows():
        ...
        # 3. SPOT-BASED STOP LOSS
        if option_type == 'CE' and spot < spot_sl_level:
            xp = close; xr = 'SPOT_SL'; xt = ts; break
...
bt.execute_tsl_idx = execute_tsl_idx_v15_spot_sl_correct
...
def main():
    print("=" * 70)
    print("RUNNING V15 CORRECT SPOT-BASED STOP LOSS BACKTEST")
    print("=" * 70)

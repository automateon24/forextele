...
# Define custom run_index with Selective 5-minute micro-confirmation (MTC)
def run_index_v15_selective_mtc(idx_name: str, opt_data: pd.DataFrame, eod_data: pd.DataFrame, cfg: bt.IndexConfig):
    print(f"  [{idx_name}] Starting backtest with Selective 5-Min MTC...", flush=True)
        ...
        for i in range(3, len(c15)):
            ...
            for strat in active_strats:
                ...
                for direction in dirs:
                    ...
                    # ── V15.0 Selective Micro-Timeframe Confirmation (MTC) ──
                    is_reversal = 'REVERSAL' in strat.name or 'MEAN' in strat.name or 'BLOCK' in strat.name or 'CRUSH' in strat.name or 'CLIMAX' in strat.name or 'LOW' in strat.name or 'HIGH' in strat.name
                    is_trend = 'TREND' in strat.name or 'BREAK' in strat.name or 'BURST' in strat.name or 'DRIVE' in strat.name or 'BOUNCE' in strat.name or 'CROSS' in strat.name or 'SCALPING' in strat.name or 'HERO' in strat.name or 'BLAST' in strat.name

                    # We ONLY apply MTC to Trend/Breakout strategies, Reversals bypass it
                    if is_trend and not is_reversal:
                        minute = hhmm % 100
                        hour = hhmm // 100
                        if minute >= 5:
...
bt.run_index = run_index_v15_selective_mtc
...
def main():
    print("=" * 70)
    print("RUNNING V15 SELECTIVE MTC BACKTEST")
    print("=" * 70)

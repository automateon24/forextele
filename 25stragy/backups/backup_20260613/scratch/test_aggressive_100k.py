...
def run_index_aggressive_100k(idx_name: str, opt_data: pd.DataFrame, eod_data: pd.DataFrame, cfg: bt.IndexConfig):
    ...
    # Aggressive stop loss circuit breaker (e.g. -15% of index capital)
    scaled_cb = -15000.0

    # Custom aggressive deploy percentages
    def get_aggressive_deploy_pct(strat_name: str) -> float:
        ...
        for i in range(3, len(c15)):
            ...
            for strat in active_strats:
                ...
                for direction in dirs:
                    ...
                    deploy_cap = bt.CAPITAL_PER_INDEX * get_aggressive_deploy_pct(strat.name)
...
bt.run_index = run_index_aggressive_100k

def run_test_aggressive():
    print("\n" + "="*70)
    print("RUNNING AGGRESSIVE 100K CAPITAL GATED ULTIMATE ENGINE")
    print("  Capital per index: Rs. 100,000 (Hard Concurrent Cap) | Max Lots Cap: 100")
    print("  Tier 1-4 Deploy Pct: 90% / 80% / 70% / 60%")
    print("="*70)
    
    # Configure in memory for Aggressive 100k
    bt.CAPITAL_PER_INDEX = 100000
    bt.MAX_LOTS_CAP = 100
    bt.DAILY_CIRCUIT_BREAKER_RS = -15000  # 15% Max index daily loss
    bt.ENABLE_REGIME_SCALING = False
    ...
    if rows:
        ...
        out.to_csv(r'C:\25stragy\backtest_results\strict_aggressive_100k_trades.csv', index=False)
        print(f"\n  Saved {len(out)} trades to C:\\25stragy\\backtest_results\\strict_aggressive_100k_trades.csv")

...
# Define execute_tsl_idx with Refined Stagnancy Cut
def execute_tsl_idx_v15_refined_stagnant(entry_bar: pd.Series, remaining: pd.DataFrame, hard_exit: int = 1430, 
    ...
    elapsed_mins = 0
    # Index-specific stagnant minutes
    stagnant_mins = 120 if index_name == 'NIFTY' else 90
    ...
    for _, bar in remaining.iterrows():
        ...
        # 4. REFINED THETA-PRESERVATION STAGNANCY CUT
        # Only cut if flat/losing (close < ep * 1.01) after stagnant_mins
        if elapsed_mins >= stagnant_mins:
            if close < ep * 1.01:
...
bt.execute_tsl_idx = execute_tsl_idx_v15_refined_stagnant
...
def main():
    print("=" * 70)
    print("RUNNING V15 REFINED STAGNANCY CUT BACKTEST")
    print("=" * 70)

...
# Top 6 strategies
top_6_strategies = ["ZERO_HERO", "GAMMA_BLAST", "MOMENTUM_BURST", "ATR_BREAK", "VWAP_BOUNCE", "MACD_DIVERGENCE"]

def run_test_profile(capital_per_index: float, max_lots_cap: int, label: str):
    print("\n" + "="*70)
    print(f"RUNNING EXPIRY-ONLY PORTFOLIO: {label}")
    ...
    for idx in bt.INDEX_CONFIGS:
        bt.ACTIVE_STRATEGIES_BY_INDEX[idx] = set(top_6_strategies)

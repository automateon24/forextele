...
def signal_check(strat: StrategyDef, direction: str,
    ...
    if n == 'ULTIMATE_DAY_HIGH_LOW':
        ...
        near_high = abs(spot - day_high) / day_high < DAY_PROXIMITY_PCT
        rejection = float(c['close']) < float(p['low'])
        if d == 'PE': return (near_high or rejection) and rsi > 58

    elif n == 'DAY_LOW_BULLISH':
        near_low = abs(spot - day_low) / day_low < DAY_PROXIMITY_PCT
...
def execute_tsl_idx(entry_bar: pd.Series, remaining: pd.DataFrame, hard_exit: int = 1430, 
    ...
    # Premium-adaptive parameters for option safety and run-room
    if ep < 40.0:
        # Cheap options (e.g. Zero Hero, Gamma Blast) - Let them run with wide trailing stop losses
        tsl_activate = max(0.40, dna.tsl_activate)
        tsl_trail = max(0.25, dna.tsl_trail)
        target_pct = max(3.50, dna.target_pct)
        sl_backstop = max(0.50, dna.sl_backstop)
    elif ep < 100.0:
        # Mid-range options
        tsl_activate = max(0.20, dna.tsl_activate)
        tsl_trail = max(0.12, dna.tsl_trail)
        target_pct = max(1.50, dna.target_pct)
        sl_backstop = max(0.40, dna.sl_backstop)
    else:
        # Expensive options (theta-decay sensitive) - Apply standard protective caps
        tsl_activate = min(0.08, dna.tsl_activate)
        tsl_trail = min(0.05, dna.tsl_trail)
        ...
        for i in range(3, len(c15)):
            ...
            for strat in active_strats:
                ...
                for direction in dirs:
                    ...
                    # Dynamic Option Chain Strike Selection based on Sweet Spot & Highest OI
                    candidates = day_data[
                    ...
                    if not candidates.empty:
                        # Find strikes with premium within our configured sweet spot
                        sweet_spot = candidates[
                            (candidates['close'] >= STRIKE_MIN_PREMIUM) & 
                            (candidates['close'] <= STRIKE_MAX_PREMIUM)
                        ]
                        if not sweet_spot.empty:
                            # Choose the candidate with the highest Open Interest (OI)
                            best_row = sweet_spot.loc[sweet_spot['oi'].fillna(0).idxmax()]
                            best_strike = best_row['strike']
                        else:
                            # Fallback: pick the strike closest to our target premium (e.g., Rs. 60)

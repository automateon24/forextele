...
# Overrides for TSL (used in execute_tsl_idx)
TSL_ACTIVATE = 0.06
TSL_TRAIL    = 0.04
SL_BACKSTOP  = 0.30
TARGET_PCT   = 0.35
...
INDEX_CONFIGS: Dict[str, IndexConfig] = {
    'NIFTY':      IndexConfig('NIFTY',      75,  50,  3, premium_scale=1.0,  hard_exit=1430, max_ce_day=2, wide_range_pts=120,  entry_cutoff=1430, slippage_pts=0.5),
    'BANKNIFTY':  IndexConfig('BANKNIFTY',  15,  100, 2, premium_scale=3.0,  hard_exit=1430, max_ce_day=2, wide_range_pts=300,  entry_cutoff=1430, slippage_pts=1.5),
    'FINNIFTY':   IndexConfig('FINNIFTY',   40,  50,  1, premium_scale=1.6,  hard_exit=1430, max_ce_day=2, wide_range_pts=120,  entry_cutoff=1430, slippage_pts=0.8),
    'SENSEX':     IndexConfig('SENSEX',     10,  100, 4, premium_scale=4.0,  hard_exit=1430, max_ce_day=2, wide_range_pts=350,  entry_cutoff=1430, slippage_pts=2.5),
}
...
INDEX_TSL_MULTIPLIERS = {
    'NIFTY':      {'activate': 1.0, 'trail': 0.6, 'target': 1.0},
    'BANKNIFTY':  {'activate': 1.2, 'trail': 0.5, 'target': 1.2},
    'FINNIFTY':   {'activate': 1.1, 'trail': 0.6, 'target': 1.1},
    'SENSEX':     {'activate': 1.3, 'trail': 0.5, 'target': 1.3},
}
...
BASE_STRATEGY_DNA = {
    ...
    'MEAN_REVERSION':        {'tsl_a': 0.06, 'tsl_t': 0.04, 'tgt': 0.35, 'sl': 0.30, 'thresh': 0.82, 'max_d': 4, 'min_p': 45,  'max_p': 600,  'boost': 0.05},
    ...
    'WIDE_RANGE_RIDER':      {'tsl_a': 0.07, 'tsl_t': 0.05, 'tgt': 0.50, 'sl': 0.30, 'thresh': 0.82, 'max_d': 3, 'min_p': 60,  'max_p': 600,  'boost': 0.03},
    'MAGIC_SQUARE':          {'tsl_a': 0.05, 'tsl_t': 0.03, 'tgt': 0.20, 'sl': 0.20, 'thresh': 0.85, 'max_d': 3, 'min_p': 100, 'max_p': 400,  'boost': 0.05},
    'SHORT_UNWIND':          {'tsl_a': 0.04, 'tsl_t': 0.02, 'tgt': 0.20, 'sl': 0.15, 'thresh': 0.90, 'max_d': 1, 'min_p': 100, 'max_p': 350,  'boost': 0.03},
    ...
    'GAMMA_BLAST':           {'tsl_a': 0.15, 'tsl_t': 0.12, 'tgt': 2.00, 'sl': 0.50, 'thresh': 0.80, 'max_d': 2, 'min_p': 5,   'max_p': 150,  'boost': 0.10},
    'ZERO_HERO':             {'tsl_a': 0.12, 'tsl_t': 0.10, 'tgt': 1.00, 'sl': 0.35, 'thresh': 0.85, 'max_d': 2, 'min_p': 5,   'max_p': 100,  'boost': 0.05},
    ...
    'TREND_FOLLOWING':       {'tsl_a': 0.04, 'tsl_t': 0.02, 'tgt': 0.20, 'sl': 0.20, 'thresh': 0.92, 'max_d': 1, 'min_p': 80,  'max_p': 400,  'boost': 0.03},
    
    # === TIER 5: NEW UNTESTED STRATEGIES (10) ===
...
ACTIVE_STRATEGIES_BY_INDEX = {
    'NIFTY': {
        'DAY_LOW_BULLISH', 'DAY_HIGH_BEARISH', 'MEAN_REVERSION', 'VOLATILITY_BREAKOUT',
        'EARLY_BREAKDOWN', 'BEAR_TREND_FOLLOWER', 'BULL_TREND_FOLLOWER', 'ORDER_BLOCK_REVERSAL',
        'WIDE_RANGE_RIDER', 'MAGIC_SQUARE', 'SHORT_UNWIND', 'ENHANCED_BEARISH',
        'VOLUME_CLIMAX', 'PUT_WRITER_SUPPORT', 'RSI_REVERSAL', 'BOLLINGER_SQUEEZE',
        'MACD_DIVERGENCE', 'ATR_BREAK', 'SCALPING', 'OPTIONS_GREEKS',
        'MOMENTUM_BURST', 'ULTIMATE_DAY_HIGH_LOW', 'GAMMA_BLAST', 'ZERO_HERO'
    },
    'BANKNIFTY': {
        'DAY_LOW_BULLISH', 'DAY_HIGH_BEARISH', 'MEAN_REVERSION', 'VOLATILITY_BREAKOUT',
        'EARLY_BREAKDOWN', 'BEAR_TREND_FOLLOWER', 'BULL_TREND_FOLLOWER', 'ORDER_BLOCK_REVERSAL',
        'WIDE_RANGE_RIDER', 'MAGIC_SQUARE', 'SHORT_UNWIND', 'ENHANCED_BEARISH',
        'BOLLINGER_SQUEEZE', 'ATR_BREAK', 'VWAP_BOUNCE', 'MOMENTUM_BURST',
        'OPTIONS_GREEKS', 'SCALPING', 'ULTIMATE_DAY_HIGH_LOW', 'AI_ENHANCED',
        'GAMMA_BLAST', 'ZERO_HERO'
    },
    'FINNIFTY': {
        'DAY_LOW_BULLISH', 'DAY_HIGH_BEARISH', 'MEAN_REVERSION', 'VOLATILITY_BREAKOUT',
        'EARLY_BREAKDOWN', 'BEAR_TREND_FOLLOWER', 'BULL_TREND_FOLLOWER', 'ORDER_BLOCK_REVERSAL',
        'WIDE_RANGE_RIDER', 'MAGIC_SQUARE', 'SHORT_UNWIND', 'ENHANCED_BEARISH',
        'RSI_REVERSAL', 'ENHANCED_BULLISH', 'DAY_HIGH_LOW_TRADITIONAL', 'PUT_WRITER_SUPPORT',
        'EMA_CROSSOVER', 'BOLLINGER_SQUEEZE', 'MACD_DIVERGENCE', 'OPTIONS_GREEKS',
        'ULTIMATE_DAY_HIGH_LOW', 'AI_ENHANCED', 'GAMMA_BLAST', 'ZERO_HERO'
    },
    'SENSEX': {
        'DAY_LOW_BULLISH', 'DAY_HIGH_BEARISH', 'MEAN_REVERSION', 'VOLATILITY_BREAKOUT',
        'EARLY_BREAKDOWN', 'BEAR_TREND_FOLLOWER', 'BULL_TREND_FOLLOWER', 'ORDER_BLOCK_REVERSAL',
        'WIDE_RANGE_RIDER', 'MAGIC_SQUARE', 'SHORT_UNWIND', 'ENHANCED_BEARISH',
        'EMA_CROSSOVER', 'VOLUME_CLIMAX', 'VWAP_BOUNCE', 'MOMENTUM_BURST', 'SCALPING',
        'GAMMA_BLAST', 'ZERO_HERO'
    }
}
...
def make_strategies_v8() -> List[StrategyDef]:
    ...
    strats = make_strategies()  # Get base 26 strategies
    
    # 10 Tier 5 strategies to complete the strategy matrix
    tier5_strats = [
        StrategyDef('MOMENTUM_BURST', 'BOTH', 'ATM', 1030, 1430,
                    sl_pct=0.25, target_pct=0.40, tsl_pts=15.0, min_premium=60, max_premium=400,
                    require_vwap=True, require_volume=True),
        StrategyDef('VWAP_BOUNCE', 'BOTH', 'ATM', 1030, 1430,
                    sl_pct=0.20, target_pct=0.30, tsl_pts=12.0, min_premium=70, max_premium=350,
                    require_vwap=True, require_volume=True),
        StrategyDef('OPENING_DRIVE', 'BOTH', 'ATM', 915, 945,
                    sl_pct=0.30, target_pct=0.50, tsl_pts=18.0, min_premium=50, max_premium=400,
                    require_vwap=True, require_volume=True),
        StrategyDef('PREMIUM_CRUSH', 'BOTH', 'ATM', 1030, 1430,
                    sl_pct=0.15, target_pct=0.20, tsl_pts=8.0, min_premium=90, max_premium=300,
                    require_vwap=False, require_volume=False),
        StrategyDef('RSI_REVERSAL', 'BOTH', 'ATM', 1030, 1430,
                    sl_pct=0.20, target_pct=0.30, tsl_pts=12.0, min_premium=60, max_premium=400,
                    require_vwap=False, require_volume=False),
        StrategyDef('EMA_CROSSOVER', 'BOTH', 'ATM', 1030, 1430,
                    sl_pct=0.25, target_pct=0.45, tsl_pts=15.0, min_premium=55, max_premium=450,
                    require_vwap=True, require_volume=True),
        StrategyDef('BOLLINGER_SQUEEZE', 'BOTH', 'ATM', 1030, 1430,
                    sl_pct=0.25, target_pct=0.40, tsl_pts=12.0, min_premium=65, max_premium=400,
                    require_vwap=True, require_volume=True),
        StrategyDef('VOLUME_CLIMAX', 'BOTH', 'ATM', 1030, 1430,
                    sl_pct=0.20, target_pct=0.35, tsl_pts=12.0, min_premium=70, max_premium=350,
                    require_vwap=False, require_volume=True),
        StrategyDef('ATR_BREAK', 'BOTH', 'ATM', 1030, 1430,
                    sl_pct=0.30, target_pct=0.55, tsl_pts=18.0, min_premium=50, max_premium=500,
                    require_vwap=True, require_volume=True),
        StrategyDef('MACD_DIVERGENCE', 'BOTH', 'ATM', 1030, 1430,
                    sl_pct=0.25, target_pct=0.40, tsl_pts=12.0, min_premium=60, max_premium=400,
                    require_vwap=False, require_volume=False)
    ]
    strats.extend(tier5_strats)
    return strats
...
def execute_fixed_target_idx(entry_bar: pd.Series, remaining: pd.DataFrame,
                             ...
                             index_name: str = 'NIFTY', strat_name: str = '') -> Tuple[float, str, object]:
...
def signal_check_idx(strat, direction: str, c15_slice, day_ohlc: dict,
    ...
    if strat.name in TIER1_ELEVEN_AM:
        if strat.name in {'TREND_FOLLOWING', 'SHORT_UNWIND'}:
            return False  # Disabled strategies
        ...
        cutoff = 1500 if (strat.name in {'GAMMA_BLAST', 'ZERO_HERO'} and expiry) else 1300
...
def run_index(idx_name: str, opt_data: pd.DataFrame,
    ...
    for day in trading_days:
        ...
        for i in range(3, len(c15)):
            ...
            # Daily Circuit Breaker check: if closed PnL today <= -10,000 Rs, stop taking entries
            ...
            for strat in active_strats:
                ...
                for direction in dirs:
                    ...
                    # Fixed 3 Lots for all trades as requested
                    actual_lots = 3
                    ...
                    if fixed_tgt:
                        xp, xr, xt = execute_fixed_target_idx(entry_bar, remaining, fixed_tgt, dynamic_exit, idx_name, strat.name)
                    else:
                        xp, xr, xt = execute_tsl_idx(entry_bar, remaining, dynamic_exit, cfg.premium_scale, regime, strat.name, idx_name)

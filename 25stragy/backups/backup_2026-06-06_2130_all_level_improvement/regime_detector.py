...
# ─────────────────────────────────────────────────────────────────────────────
# Thresholds — tuned from 2025/2026 backtest data
# ─────────────────────────────────────────────────────────────────────────────
TREND_STRONG_MOVE_PTS    = 150   # spot moved ≥150 pts from open → trending (75th pct)
TREND_STRONG_MOVE_FAST   = 80    # …in first 30 mins → very fast trend
HIGH_VOL_RANGE_PTS       = 400   # daily range ≥400 pts → high-vol (90th pct = 396)
RANGE_MAX_TREND_PTS      = 50    # spot ≤50 pts from open → range-bound (25th pct)
RANGE_MAX_RANGE_PTS      = 180   # AND total range ≤180 pts
HIGH_IV_THRESHOLD        = 20.0  # avg IV ≥ 20.0 (20%, stored as raw %) → high vol
UDHL_BLOCK_MOVE_PTS      = 100   # UDHL disabled if spot already moved >100 pts
...
# Which strategies are compatible with each regime
...
class RegimeDetector:
    ...
    def _classify(self, ctx: DayContext, spot: float, hhmm: int) -> RegimeSnapshot:
        ...
        # UDHL-specific block: if spot already moved >0.43% in either direction (equivalent to 100 pts on Nifty 23k)
        udhl_blocked = False
        udhl_reason  = ""
        udhl_threshold = ctx.day_open * 0.0043
    ...
    def _classify_from_stats(
        ...
        trend_strong_move = day_open * 0.0065  # Nifty 150 pts ≈ 0.65%
        high_vol_range    = day_open * 0.0174  # Nifty 400 pts ≈ 1.74%
        range_max_trend   = day_open * 0.0022  # Nifty 50 pts  ≈ 0.22%
        range_max_range   = day_open * 0.0078  # Nifty 180 pts ≈ 0.78%

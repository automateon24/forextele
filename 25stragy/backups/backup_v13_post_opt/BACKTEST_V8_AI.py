...
TRADEABLE_REGIMES = {'NORMAL', 'TRENDING_BEAR', 'TRENDING_BULL', 'RANGE_BOUND', 'HIGH_VOLATILITY'}
...
CAPITAL_BASE = config_db["system"].get("capital_base", 400000)
CAPITAL = CAPITAL_BASE
MAX_TEST_LOTS = config_db["system"].get("max_test_lots", 3)
LOT_MULTIPLIER = config_db["system"].get("lot_multiplier", 1.0)
MAX_LOTS_CAP = config_db["system"].get("max_lots_cap", 15)

# Capital-based position sizing and risk management settings
CAPITAL_PER_INDEX = config_db["system"].get("capital_per_index", 100000)
TIER1_DEPLOY_PCT = config_db["system"].get("tier1_deploy_pct", 0.25)
TIER2_DEPLOY_PCT = config_db["system"].get("tier2_deploy_pct", 0.20)
TIER3_DEPLOY_PCT = config_db["system"].get("tier3_deploy_pct", 0.12)
TIER4_DEPLOY_PCT = config_db["system"].get("tier4_deploy_pct", 0.08)
...
DAILY_CIRCUIT_BREAKER_RS = config_db["system"].get("daily_circuit_breaker_rs", -15000)

STRATEGY_TUNING = config_db.get("strategy_tuning", {})
MAX_TRADE_DURATION_MINS = STRATEGY_TUNING.get("max_trade_duration_mins", 60)
ORB_CANDLES = STRATEGY_TUNING.get("orb_candles", 4)
DAY_PROXIMITY_PCT = STRATEGY_TUNING.get("day_proximity_pct", 0.004)
MEAN_REVERSION_BB_SD = STRATEGY_TUNING.get("mean_reversion_bb_sd", 1.5)
ULTIMATE_DAY_HIGH_LOW_PROXIMITY_PCT = STRATEGY_TUNING.get("ultimate_day_high_low_proximity_pct", 0.0015)
STRIKE_MIN_PREMIUM = STRATEGY_TUNING.get("strike_min_premium", 15.0)
STRIKE_MAX_PREMIUM = STRATEGY_TUNING.get("strike_max_premium", 150.0)
STRIKE_TARGET_PREMIUM = STRATEGY_TUNING.get("strike_target_premium", 60.0)
...
def signal_check(strat: StrategyDef, direction: str,
    ...
    if n == 'ULTIMATE_DAY_HIGH_LOW':
        ...
        near_low  = prev_lo <= run_low * (1.0 + ULTIMATE_DAY_HIGH_LOW_PROXIMITY_PCT)
        near_high = prev_hi >= run_high * (1.0 - ULTIMATE_DAY_HIGH_LOW_PROXIMITY_PCT)
        ...
        near_high_wide = abs(spot - day_high) / day_high < (DAY_PROXIMITY_PCT * 1.8)
        rejection = float(c['close']) < float(p['low'])
        if d == 'PE': return near_high_wide and rejection and rsi > 58

    elif n == 'DAY_LOW_BULLISH':
        near_low_wide = abs(spot - day_low) / day_low < (DAY_PROXIMITY_PCT * 1.8)
        bounce   = float(c['close']) > float(p['high'])
        if d == 'CE': return near_low_wide and bounce and (rsi < 47 or pcr > 1.2)

    elif n == 'DAY_HIGH_LOW_TRADITIONAL':
        if len(candles15) < (ORB_CANDLES + 1):
            return False
        first_hour = candles15.iloc[:ORB_CANDLES]
        ...
        bearish = (ema5 < ema20 and pcr < 1.0 and rsi < 48 and c['close'] < c['open'])
        bullish = (ema5 > ema20 and pcr > 1.3 and rsi > 52 and c['close'] > c['open'])
        ...
        if n_bars >= 5:
            ...
            if bb_std == 0:
                return False
            bb_up = bb_mid + MEAN_REVERSION_BB_SD * bb_std
            bb_dn = bb_mid - MEAN_REVERSION_BB_SD * bb_std
        ...
        if d == 'CE' and len(candles15) >= ORB_CANDLES:
            first_hour = candles15.iloc[:ORB_CANDLES]
            orb_high = float(first_hour['high'].max())
        ...
        if d == 'PE' and len(candles15) >= ORB_CANDLES:
            first_hour = candles15.iloc[:ORB_CANDLES]
            orb_low = float(first_hour['low'].min())
...
def get_dynamic_hard_exit(index: str, strategy: str, regime: str, is_expiry: bool) -> int:
    ...
    is_blast = strategy in ['ZERO_HERO', 'GAMMA_BLAST', 'SCALPING', 'MORNING_BREAKOUT', 'BREAKOUT']
    if is_expiry:
        if is_blast:
            return 1510
        return 1330 if is_reversal else 1430
    if regime in ['TRENDING_BULL', 'TRENDING_BEAR', 'EXPLOSIVE_GAP']:
        return 1345 if is_reversal else 1430
    return 1330 if is_reversal else 1430
...
def execute_fixed_target_idx(entry_bar: pd.Series, remaining: pd.DataFrame,
                             ...
                             regime: str = 'NORMAL', is_expiry: bool = False) -> Tuple[float, str, object]:
    dna = get_index_strategy_dna(index_name, strat_name)
    ep  = float(entry_bar['open'])
    
    # Premium-adaptive adjustments for cheap option contracts
    sl_backstop = dna.sl_backstop
    if ep < 40.0:
        sl_backstop = max(0.50, sl_backstop)
        target_pct = max(3.50, target_pct)
    elif ep < 100.0:
        sl_backstop = max(0.40, sl_backstop)
        target_pct = max(1.50, target_pct)
    else:
    ...
    for _, bar in remaining.iterrows():
        ...
        is_winning = (float(bar['close']) > ep)
        vol_factor = 2.0 if (regime in ['HIGH_VOLATILITY', 'EXPLOSIVE_GAP']) else 1.0
        allowed_duration = MAX_TRADE_DURATION_MINS * (3.0 if (is_expiry or is_winning) else 1.0) * vol_factor
        
        if mins_in_trade >= allowed_duration:
...
def execute_tsl_idx(entry_bar: pd.Series, remaining: pd.DataFrame, hard_exit: int = 1430, 
    ...
    # Trailing Stop Loss parameters strictly based on DNA
    tsl_activate = dna.tsl_activate
    tsl_trail = dna.tsl_trail
    target_pct = dna.target_pct
    sl_backstop = dna.sl_backstop
    
    # Premium-adaptive safety overrides for extremely cheap options
    if ep < 40.0:
        tsl_activate = max(tsl_activate, 0.40)
        tsl_trail = max(tsl_trail, 0.25)
        target_pct = max(target_pct, 3.50)
        sl_backstop = max(sl_backstop, 0.50)
...
def signal_check_idx(strat, direction: str, c15_slice, day_ohlc: dict,
                     ...
                     real_prem: float, cfg: IndexConfig, regime: str = 'NORMAL', day: str = '',
                     idx_profiles: dict = None) -> bool:
    ...
    # 2. Apply the DNA-driven Filters
    
    # Respect the strategy's DNA cutoff (entry_end)
    ...
    # We apply the cutoff filter
    if hhmm > cutoff:
        return False
    
    # Reversal Volume check: only apply if the strategy DNA says require_volume is True
    if strat.require_volume:
        if not volume_spike_filter(c15_slice, min_spike=1.3):
            return False
    
    # ADX + BB position check for mean reversion (keep standard mean reversion checks)
    ...
    # EMA alignment check based on strategy profile's ema_structure (instead of hardcoded TREND_FOLLOWERS list)
    if idx_profiles:
        profile = idx_profiles.get(strat.name)
        if profile and profile.ema_structure != 'ANY':
...
def run_index(idx_name: str, opt_data: pd.DataFrame,
    ...
    print(f"  [{idx_name}] Grouping option data by date...", flush=True)
    grouped_opt = {date: grp for date, grp in opt_data.groupby('date')}
    print(f"  [{idx_name}] Grouping EOD data by dt...", flush=True)
    grouped_eod = {row['dt']: pd.DataFrame([row]) for _, row in eod_data.iterrows()} if not eod_data.empty else {}
    ...
    for day in trading_days:
        ...
        for i in range(3, len(c15)):
            ...
            for strat in active_strats:
                ...
                # Dynamic strategy regime compatibility check from strategy_dna.json
                ...
                for direction in dirs:
                    ...
                    # Threshold check strictly based on DNA entry_threshold
                    dna = get_index_strategy_dna(idx_name, strat.name)
                    min_conf = dna.entry_threshold
                    
                    if regime == 'EXPLOSIVE_GAP':
                        min_conf = max(0.50, min_conf - 0.05)
                        conf = min(0.95, conf + 0.08)
                    
                    if conf < min_conf:
                        continue

                    # Dynamic Option Chain Strike Selection (Cheapest Premium among High-OI Candidates)
                    candidates = day_data[
                    ...
                    if not candidates.empty:
                        # Determine strategy-specific scaled premium range
                        min_p = min(STRIKE_MIN_PREMIUM, strat.min_premium * cfg.premium_scale)
                        max_p = max(STRIKE_MAX_PREMIUM, strat.max_premium * cfg.premium_scale)
                        ...
                        if not sweet_spot.empty:
                            ...
                            if max_oi > 0:
                                # Keep liquid options with at least 40% of max OI in the sweet spot
                                liquid_candidates = sweet_spot[sweet_spot['oi'].fillna(0) >= max_oi * 0.40]
                                # Pick the one with the lowest close price (cheapest premium for gamma blast)
                                best_row = liquid_candidates.loc[liquid_candidates['close'].idxmin()]
                                best_strike = best_row['strike']
                            else:
                                # Fallback: pick the strike closest to target premium
                                candidates_sorted = sweet_spot.iloc[(sweet_spot['close'] - STRIKE_TARGET_PREMIUM).abs().argsort()]
                                best_strike = candidates_sorted.iloc[0]['strike']
                        else:
                            # Fallback: pick the strike closest to target premium
                            candidates_sorted = candidates.iloc[(candidates['close'] - STRIKE_TARGET_PREMIUM).abs().argsort()]
                            best_strike = candidates_sorted.iloc[0]['strike']

                    # Retrieve the specific option data for the best_strike selected
                    opt_b = day_data[
                        (day_data['option_type_flag'] == direction) &
                    ...
                    # Capital-based dynamic position sizing (Risk Management)
                    ...
                    if fixed_tgt:
                        xp, xr, xt = execute_fixed_target_idx(entry_bar, remaining, fixed_tgt, dynamic_exit, idx_name, strat.name, regime, expiry)
                    else:

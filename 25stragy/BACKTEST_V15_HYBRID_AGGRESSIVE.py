...
if not os.path.exists(CONFIG_PATH):
    CONFIG_PATH = "config_aggressive_100k.json"
...
print(f"[DNA MATRIX] Loaded {len(ACTIVE_STRATEGIES)} strategies x {len(INDEX_CONFIGS)} indices = {len(INDEX_STRATEGY_DNA_MATRIX)} DNA combinations")
...
def execute_fixed_target_idx(entry_bar: pd.Series, remaining: pd.DataFrame,
    ...
    if ENABLE_REGIME_SCALING or index_name == 'NIFTY':
        if 'TRENDING' in regime:
            mult = 2.5 if strat_name in ['MOMENTUM_BURST', 'MACD_DIVERGENCE'] else 2.0
            target_pct *= mult
...
def execute_tsl_idx(entry_bar: pd.Series, remaining: pd.DataFrame, hard_exit: int = 1430, 
    ...
    is_expiry_special = is_expiry and (strat_name in ['ZERO_HERO', 'GAMMA_BLAST'])
    if is_expiry_special:
        tsl_activate = 4.0   # Activate trailing stop only after a 400% gain (5x premium)
        tsl_trail = 0.50     # Give it a wide 50% breathing room to handle wiggles
        target_pct = 999.0   # Let it run!
        sl_backstop = dna.sl_backstop
    else:
...
def stitch_remaining_bars(entry_bar, remaining_timestamps, day_lookup, direction, atm_step):
    entry_spot = float(entry_bar['spot'])
    entry_atm = round(entry_spot / atm_step) * atm_step
    strike_str = entry_bar['strike']
    
    if strike_str == 'ATM':
        K = entry_atm
    elif strike_str.startswith('ATM+'):
        offset = int(strike_str.replace('ATM+', ''))
        K = entry_atm + offset * atm_step
    elif strike_str.startswith('ATM-'):
        offset = int(strike_str.replace('ATM-', ''))
        K = entry_atm - offset * atm_step
    else:
        try: K = float(strike_str)
        except: K = entry_atm
    ...
    for ts in remaining_timestamps:
        spot_t = None
        try:
            r_atm = day_lookup.loc[(ts, direction, 'ATM')]
            spot_t = float(r_atm['spot'])
        except KeyError:
            for s in ['ATM+1', 'ATM-1', 'ATM+2', 'ATM-2', 'ATM+3', 'ATM-3']:
                try:
                    r_other = day_lookup.loc[(ts, direction, s)]
                    spot_t = float(r_other['spot'])
                    break
                except KeyError:
                    continue
        ...
        atm_t = round(spot_t / atm_step) * atm_step
        offset_t = int(round((K - atm_t) / atm_step))
        
        if offset_t == 0:
            rel_strike = 'ATM'
        elif offset_t > 0:
            rel_strike = f'ATM+{offset_t}'
        else:
            rel_strike = f'ATM{offset_t}'
            
        try:
            row_t = day_lookup.loc[(ts, direction, rel_strike)]
            new_row = row_t.to_dict()
            new_row['ts_ist'] = ts
            new_row['strike'] = strike_str
            stitched_rows.append(new_row)
        except KeyError:
            if direction == 'CE':
                intrinsic = max(0.05, spot_t - K)
            else:
                intrinsic = max(0.05, K - spot_t)
            try:
                new_row = day_lookup.loc[(ts, direction, 'ATM')].to_dict()
            except KeyError:
                new_row = {'spot': spot_t, 'volume': 0, 'open_interest': 0}
            new_row['strike'] = strike_str
            new_row['open'] = intrinsic
            new_row['high'] = intrinsic
            new_row['low'] = intrinsic
            new_row['close'] = intrinsic
            new_row['ts_ist'] = ts
            stitched_rows.append(new_row)
...
def run_index(idx_name: str, opt_data: pd.DataFrame,
    ...
    for day in trading_days:
        ...
        day_lookup = day_data.drop_duplicates(subset=['ts_ist', 'option_type_flag', 'strike']).set_index(['ts_ist', 'option_type_flag', 'strike'])
        ...
        for i in range(3, len(c15)):
            ...
            for strat in active_strats:
                ...
                for direction in dirs:
                    ...
                    # Mirror the strike offset for PE option type to get OTM strike
                    target_strike = strat.strike
                    if direction == 'PE' and target_strike.startswith('ATM+'):
                        target_strike = target_strike.replace('ATM+', 'ATM-')
                    elif direction == 'PE' and target_strike.startswith('ATM-'):
                        target_strike = target_strike.replace('ATM-', 'ATM+')

                    opt_b = day_data[
                        (day_data['option_type_flag'] == direction) &
                        (day_data['strike'] == target_strike) &
                        (day_data['hhmm'] == hhmm)
                    ]
                    if len(opt_b) == 0:
                        continue
                    ...
                    # STRIKE STITCHING: Reconstruct remaining bars to track fixed strike contract
                    remaining_timestamps = exec_bars['ts_ist'].iloc[1:].tolist()
                    remaining = stitch_remaining_bars(entry_bar, remaining_timestamps, day_lookup, direction, cfg.atm_step)
                    if remaining.empty:
                        continue
...
def run_portfolio_sequential(datasets: Dict[str, Tuple[pd.DataFrame, pd.DataFrame, IndexConfig]]) -> Dict[str, List[Trade]]:
    all_dates = sorted(list(set().union(*(datasets[name][0]['date'].unique() for name in datasets))))
    num_days_limit = os.environ.get("NUM_DAYS_LIMIT")
    if num_days_limit:
        all_dates = all_dates[:int(num_days_limit)]

    print(f"\n[SEQUENTIAL SIMULATION] Processing {len(all_dates)} trading days sequentially with global capital sharing...")

    day_regimes_by_idx = {}
    for idx_name, (opt, _, _) in datasets.items():
        print(f"  [{idx_name}] Labelling regimes on {opt['date'].nunique()} days...", flush=True)
        day_regimes_by_idx[idx_name] = label_days(opt)

    all_trades: List[Trade] = []
    prev_closes = {idx_name: 0.0 for idx_name in datasets}
    ...
    for day_idx, day in enumerate(all_dates):
        ...
        pcr_by_idx = {}
        expiry_by_idx = {}
        active_strats_by_idx = {}
        day_ohlc_by_idx = {}

        for idx_name, (opt_data, eod_data, cfg) in datasets.items():
            regime = day_regimes_by_idx[idx_name].get(day, 'NORMAL')
            if regime not in TRADEABLE_REGIMES:
                eod_row = eod_data[eod_data['dt'] == day] if not eod_data.empty else pd.DataFrame()
                if not eod_row.empty:
                    prev_closes[idx_name] = float(eod_row.iloc[0]['close'])
                continue
            ...
            day_data_by_idx[idx_name] = day_data
            day_lookup_by_idx[idx_name] = day_data.drop_duplicates(subset=['ts_ist', 'option_type_flag', 'strike']).set_index(['ts_ist', 'option_type_flag', 'strike'])
            c15 = build_15min_spot(day_data)
            if len(c15) < 4:
                continue
            c15_by_idx[idx_name] = c15

            pcr_by_idx[idx_name] = calc_pcr(day_data)
            expiry = (day.weekday() == cfg.expiry_dow) if hasattr(day, 'weekday') else False
            expiry_by_idx[idx_name] = expiry

            if expiry:
                allowed_strategies = {"ZERO_HERO", "GAMMA_BLAST"}
            else:
                allowed_strategies = ACTIVE_STRATEGIES_BY_INDEX.get(idx_name, set())
            active_strats_by_idx[idx_name] = [s for s in make_strategies_v8() if s.name in allowed_strategies]

            eod_row = eod_data[eod_data['dt'] == day] if not eod_data.empty else pd.DataFrame()
            if not eod_row.empty:
                r = eod_row.iloc[0]
                day_ohlc_by_idx[idx_name] = {k: float(r[k]) for k in ('open','high','low','close')}
            else:
                day_ohlc_by_idx[idx_name] = {
                    'open':  float(c15.iloc[0]['close']),
                    'high':  float(c15['high'].max()),
                    'low':   float(c15['low'].min()),
                    'close': float(c15.iloc[-1]['close'])
                }
        ...
        max_bars = max(len(c15) for c15 in c15_by_idx.values())
        trades_today_by_idx = {idx_name: defaultdict(int) for idx_name in datasets}
        strat_trades_by_idx = {idx_name: defaultdict(int) for idx_name in datasets}

        for i in range(3, max_bars):
            for idx_name in list(c15_by_idx.keys()):
                c15 = c15_by_idx[idx_name]
                if i >= len(c15):
                    continue
                ...
                ts = _get_ts(row)
                hhmm = ts.hour * 100 + ts.minute
                cfg = datasets[idx_name][2]

                if hhmm < 945 or hhmm > cfg.entry_cutoff:
                    continue

                today_pnl = sum(t.pnl_rs for t in all_trades if t.date == day and t.index == idx_name)
                if today_pnl <= DAILY_CIRCUIT_BREAKER_RS:
                    continue

                global_active_capital = 0.0
                for t in all_trades:
                    if t.date == day:
                        if t.entry_time <= ts < t.exit_time:
                            global_active_capital += t.entry_price * INDEX_CONFIGS[t.index].lot_size * t.lots

                avail_capital = max(0.0, CAPITAL_BASE - global_active_capital)
                if avail_capital <= 1000.0:
                    continue

                opt_day = day_data_by_idx[idx_name]
                day_lookup = day_lookup_by_idx[idx_name]
                pcr = pcr_by_idx[idx_name]
                expiry = expiry_by_idx[idx_name]
                active_strats = active_strats_by_idx[idx_name]
                day_ohlc = day_ohlc_by_idx[idx_name]
                prev_close = prev_closes[idx_name]
                regime = day_regimes_by_idx[idx_name].get(day, 'NORMAL')
                ...
                for strat in active_strats:
                    if strat.name not in INDEX_PROFILES[idx_name]:
                        continue
                    ...
                    if not compat:
                        continue
                    if strat.name in ONE_TRADE_STRATS and strat_trades_by_idx[idx_name][strat.name] >= 1:
                        continue

                    dirs = ['CE','PE'] if strat.direction == 'BOTH' else [strat.direction]

                    for direction in dirs:
                        if direction == 'CE' and trades_today_by_idx[idx_name]['CE'] >= cfg.max_ce_day:
                            continue
                        if direction == 'PE' and trades_today_by_idx[idx_name]['PE'] >= 15:
                            continue

                        profile = INDEX_PROFILES[idx_name][strat.name]
                        armed, conf, arm_reason = match_profile(profile, ctx, state, direction)
                        if not armed:
                            continue
                        ...
                        # OTM Strike Shifting for Expiry Strategies (Point 2)
                        target_strike = strat.strike
                        if expiry and strat.name == 'ZERO_HERO':
                            target_strike = 'ATM+3'
                        elif expiry and strat.name == 'GAMMA_BLAST':
                            target_strike = 'ATM+2'
                        ...
                        opt_b = opt_day[
                            (opt_day['option_type_flag'] == direction) &
                            (opt_day['strike'] == target_strike) &
                            (opt_day['hhmm'] == hhmm)
                        ]
                        if len(opt_b) == 0:
                            continue
                        ...
                        deploy_cap = CAPITAL_BASE * get_tier_deploy_pct(strat.name)
                        deploy_cap = min(deploy_cap, avail_capital)
                        actual_lots = max(1, min(MAX_LOTS_CAP, int(deploy_cap / (prem * cfg.lot_size))))

                        required_margin = prem * cfg.lot_size * actual_lots
                        if global_active_capital + required_margin > CAPITAL_BASE:
                            continue
                        ...
                        global_active_capital += required_margin
                        avail_capital = max(0.0, CAPITAL_BASE - global_active_capital)
                        trades_today_by_idx[idx_name][direction] += 1
                        strat_trades_by_idx[idx_name][strat.name] += 1
                        break

        for idx_name, (_, eod_data, _) in datasets.items():
            eod_row = eod_data[eod_data['dt'] == day] if not eod_data.empty else pd.DataFrame()
            if not eod_row.empty:
                prev_closes[idx_name] = float(eod_row.iloc[0]['close'])

    results: Dict[str, List[Trade]] = {idx_name: [] for idx_name in datasets}
    for t in all_trades:
        results[t.index].append(t)
...
def report_multi(results: Dict[str, List[Trade]], total_days: int):
    ...
    print(f"  {len(results)} indices × 36 strategies × Dynamic lot sizing")
    print(f"{'='*70}")
    ...
    print(hdr)
    print(f"  {'-'*65}")
    for idx_name in sorted(results.keys()):
...
if __name__ == '__main__':
    ...
    # ── Step 2: Run sequential portfolio simulation with capital sharing ─────
    print(f"\n{'='*70}")
    print("Running portfolio backtest in sequential mode...")
    print(f"{'='*70}")

    results = run_portfolio_sequential(datasets)
    ...
    if rows:
        ...
        out.to_csv('backtest_results/aggressive_100k_trades.csv', index=False)
        print(f"\n  Saved {len(out)} trades -> backtest_results/aggressive_100k_trades.csv")

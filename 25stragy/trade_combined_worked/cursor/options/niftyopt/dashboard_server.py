    ...
    try:
        if os.path.exists(r"C:\25stragy\telegram_signals.xlsx"):
            ...
            for _, r in tdf.iterrows():
                ...
                chan_name = "MCX Signals" if "2231238486" in chan_id else ("Equity/BTST" if "2412774015" in chan_id else "Premium Channel")
                ...
                if status in ["CLOSED_SL", "MANUAL_BOOK", "T3_HIT", "TSL_HIT_AT_COST", "TSL_HIT_AT_T1", "SL_HIT"]:
                    tele_realized += pnl_val
                    telegram_completed.append(trade_obj)
                else:
                    tele_margin += cap_utilized
                    # Estimate unrealized pnl based on status
                    try:
                        entry = float(re.search(r'[\d\.]+', trade_obj["entry_price"]).group())
                        lot_size = cap_utilized / entry if entry > 0 else 25
                        if status == "T1_HIT":
                            tgt = float(re.search(r'[\d\.]+', str(r.get("target", "0"))).group())
                            trade_obj["unrealized_pnl"] = ((tgt - entry) * 0.5) * lot_size
                        elif status == "T2_HIT":
                            tgt = float(re.search(r'[\d\.]+', str(r.get("target", "0"))).group())
                            trade_obj["unrealized_pnl"] = ((tgt - entry) * 0.8) * lot_size
                    except:
                        pass
            ...
            function renderDashboard(data) {
                ...
                if (activeHead) {
                    if (currentEngine === 'telegram') {
                        activeHead.innerHTML = `<tr><th>Time</th><th>Channel ID</th><th>Instrument</th><th>Action</th><th>Lots</th><th>Entry Range</th><th>Target / SL</th><th>Status</th><th>Unrealized PnL (Rs)</th></tr>`;
                    } else {
                ...
                if (completedHead) {
                    if (currentEngine === 'telegram') {
                        completedHead.innerHTML = `<tr><th>Exit Time</th><th>Channel ID</th><th>Instrument</th><th>Action</th><th>Lots</th><th>Entry Range</th><th>Exit State</th><th>Status</th><th>Realized PnL (Rs)</th></tr>`;
                    } else {
                ...
                if (activeTrades.length === 0) {
                    ...
                    activeTrades.forEach(t => {
                        let row = '';
                        if (currentEngine === 'telegram') {
                            const pnlClass = (t.unrealized_pnl || 0) >= 0 ? 'price-up' : 'price-down';
                            row = `
                                <tr>
                                    <td style="font-size: 11px; color: var(--text-muted);">Entry: ${t.entry_time || 'N/A'}<br>Exit: ${t.exit_time || '-'}</td>
                                    <td>
                                      <strong style="color: var(--accent-blue); font-size: 11px;">${t.index}</strong><br>
                                      <span style="font-size: 10.5px; font-weight: 500; color: #94a3b8;">${t.channel_name || 'Premium'}</span>
                                    </td>
                                    ...
                                    <td class="pnl-cell ${pnlClass}">${formatCurrency(t.unrealized_pnl || 0)}</td>
                                </tr>
                            `;
                        } else {
                        ...
                        if (currentEngine === 'telegram') {
                            row = `
                                <tr>
                                    ...
                                    <td class="pnl-cell ${pnlClass}">${formatCurrency(t.pnl_rs || 0)}</td>
                                </tr>
                            `;
                        } else {
                            row = `
                                <tr>
                                    <td style="color: var(--text-muted); font-size: 12.5px;">${t.exit_time || '-'}</td>

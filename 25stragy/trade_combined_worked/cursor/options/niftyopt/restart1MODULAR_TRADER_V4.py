...
class Config:
    ...
    MAGIC_MAX_OPEN = 3                   # V4 FIX: Hard cap at 3 (was 10) - April 30 lesson
    ...
    BIAS_FLIP_LOSS_TRIGGER = -5_000      # If P&L < -5000 in first 60min, flip bias
    ...
    PRICE_MOMENTUM_THRESHOLD = 50          # Standard threshold
    ...
    def analyze(self, data: MarketData) -> Optional[Signal]:
        ...
        if Config.TREND_VIX_OR_MOVE:
            ...
            if not (vix_ok or move_ok):
                log.info(f"[TREND] Blocked - VIX {data.vix:.1f}<{Config.CHOPPY_VIX_THRESHOLD} AND move {move_points:.0f}pt < {Config.TREND_MIN_MOVE_POINTS}pt")
                return None
        ...
        if c:
            ...
            return Signal(self.name, f"GAP_UP_TREND" if gap_pct > 0 else "GAP_DOWN_TREND", 
                         direction, c, 0.70,
                         f"Gap {direction} trend: open {data.day_open:.1f} vs prev {data.prev_close:.1f}")
        return None
...
class AIEnhancedModule(StrategyModule):
    ...
    def analyze(self, data: MarketData) -> Optional[Signal]:
        ...
        # V4: Return signals with confidence - momentum filter handles the rest
        if bull_score >= 0.65:
...
class EnhancedBullishModule(StrategyModule):
    ...
    def analyze(self, data: MarketData) -> Optional[Signal]:
        ...
        if data.rsi14 <= 30:
            ...
            return None
        if data.day_open and (data.spot - data.day_open) < -50:
...
class ScalpingModule(StrategyModule):
    ...
    def analyze(self, data: MarketData) -> Optional[Signal]:
        ...
        if all_up and strong_momentum and meaningful:
            if data.pcr_bias == 'BEARISH' or day_move < -80:
                return None
        ...
        if all_down and strong_momentum and meaningful:
            if data.pcr_bias == 'BULLISH' or day_move > 80:
                return None
...
class TradeManager:
    ...
    def can_enter(self, module: StrategyModule, direction: str, data: MarketData = None,
        ...
        if now.hour < Config.NO_ENTRY_BEFORE[0] or (now.hour == Config.NO_ENTRY_BEFORE[0] and now.minute < Config.NO_ENTRY_BEFORE[1]):
            return False
        ...
        if module.name in Config.CHOPPY_BLOCK_STRATEGIES:
            if now.hour > Config.CHOPPY_START[0] or (now.hour == Config.CHOPPY_START[0] and now.minute >= Config.CHOPPY_START[1]):
                if data and data.vix and data.vix < Config.CHOPPY_VIX_THRESHOLD:
                    log.info(f"[CHOPPY_FILTER] Blocking {module.name} - Afternoon + VIX {data.vix:.1f} < {Config.CHOPPY_VIX_THRESHOLD}")
                    return False
        
        # Magic Square: Allow multiple but check heat
        if module.name == 'MAGIC_SQUARE':
            if module.trade_count >= 10:
                return False
            # V4: Check portfolio heat - max 3 open
        ...
        # V4: Price Momentum Filter with Confidence Bypass
        if Config.PRICE_MOMENTUM_ENABLED and data and data.day_open:
            price_change = data.spot - data.day_open
            
            # V4: High confidence bypasses momentum filter
            if signal_confidence >= Config.PRICE_MOMENTUM_CONF_BYPASS:
                log.info(f"[MOMENTUM_BYPASS] {module.name} confidence {signal_confidence:.0%} >= {Config.PRICE_MOMENTUM_CONF_BYPASS:.0%} - allowing entry despite {price_change:.0f}pt move")
            else:
    ...
    def enter(self, signal: Signal, module: StrategyModule, data: MarketData = None) -> Optional[Trade]:
        ...
        now = datetime.now()
        size_multiplier = 1.0
        if now.hour > Config.FULL_SIZE_WINDOW[2] or (now.hour == Config.FULL_SIZE_WINDOW[2] and now.minute >= Config.FULL_SIZE_WINDOW[3]):
            size_multiplier = Config.REDUCED_SIZE_PCT
            log.info(f"[SIZING] {module.name} using {Config.REDUCED_SIZE_PCT*100:.0f}% size (after 14:00)")
        
        adjusted_qty = int(Config.LOT_SIZE * size_multiplier)
        ...
        size_msg = f" ({Config.REDUCED_SIZE_PCT*100:.0f}% size)" if size_multiplier < 1.0 else ""
...
class DataFeed:
    ...
    def __init__(self):
        ...
        self.data = MarketData(timestamp=datetime.now(), spot=0.0, day_open=None, 
                              day_high=None, day_low=None, prev_close=None, vix=None)
        self.closes: List[float] = []
        self._spot_history: List[float] = []  # V4 FIX: accumulate spot prices as synthetic candles
        self._last_good_chain: Dict = {}     # V4 FIX: cache last successful chain fetch
        self.connect()
    ...
    def connect(self):
        try:
            ...
            self.client = dhanhq(Config.CLIENT_ID, access_token)
            self._token_loaded_at = datetime.now()
            log.info("[DATAFEED] ✅ Dhan API connected - token loaded")
        except Exception as e:
            log.error(f"[DATAFEED] ❌ Connection failed: {e}")
            raise

    def _reconnect_if_stale(self):
        """Reload token from file and reconnect - called if spot stays 0"""
        try:
            with open(Config.TOKEN_FILE, 'r') as f:
                tokens = json.load(f)
            new_token = tokens.get('access_token', '')
            self.client = dhanhq(Config.CLIENT_ID, new_token)
            log.info("[DATAFEED] 🔄 Reconnected with fresh token from file")
        except Exception as e:
            log.error(f"[DATAFEED] ❌ Reconnect failed: {e}")
    
    def update(self, candles: List[dict]):
        now = time.time()
        do_slow = (now - self.last_slow_update) > 30
        # Auto-reconnect if spot has been 0 for >5 minutes (stale/expired token)
        if self.data.spot == 0 and (now - getattr(self, '_zero_spot_since', now)) > 300:
            log.warning("[DATAFEED] spot=0 for 5min - reloading token and reconnecting")
            self._reconnect_if_stale()
            self._zero_spot_since = now
        if self.data.spot == 0 and not hasattr(self, '_zero_spot_since'):
            self._zero_spot_since = now
        if self.data.spot > 0:
            self._zero_spot_since = now
        ...
        if candles:
            ...
            self._spot_history = self.closes.copy()
            self.data.closes = self.closes
            if self.closes:
                self.data.spot = self.closes[-1]
    ...
    def _slow_update(self):
        try:
            # NIFTY spot
            r = self.client.ticker_data(securities={Config.NIFTY_EXCHANGE: [Config.NIFTY_SECURITY_ID]})
            if r and r.get("status") == "success":
                d = r["data"]["data"][Config.NIFTY_EXCHANGE][str(Config.NIFTY_SECURITY_ID)]
                self.data.spot = float(d["last_price"])
                if not self.data.day_open:
                    self.data.day_open = float(d.get("open", self.data.spot))
                self.data.day_high = float(d.get("high", self.data.spot))
                self.data.day_low = float(d.get("low", self.data.spot))
                self.data.prev_close = float(d.get("prev_close", self.data.spot))
        except Exception as e:
            log.warning(f"[DATAFEED] Spot error: {e}")
            
        time.sleep(0.5)
        
        # Option chain
        chain, expiry = self._fetch_option_chain(self.data.spot)
        if chain:
            self.data.chain = chain
            self.data.atm_strike = self._atm_strike(self.data.spot)
            
        # PCR - use last good chain as fallback if current fetch failed
        if chain:
            self._last_good_chain = chain
        elif self._last_good_chain:
            chain = self._last_good_chain
            self.data.chain = chain
            log.info("[DATAFEED] Using cached chain (API returned empty)")
        if chain:
            self._calculate_pcr(chain)
            
        # VIX
        try:
            r = self.client.ticker_data(securities={"IDX_I": [21]})
            if r and r.get("status") == "success":
                self.data.vix = float(r["data"]["data"]["IDX_I"]["21"]["last_price"])
        except:
            pass
            
        # V4 FIX: accumulate spot as synthetic closes (one per slow_update = every 30s)
        if self.data.spot > 0:
            self._spot_history.append(self.data.spot)
            if len(self._spot_history) > 500:  # keep last 500 readings (~4hrs)
                self._spot_history = self._spot_history[-500:]
            self.closes = self._spot_history
            self.data.closes = self.closes

        # Calculate indicators
        if self.closes:
            self.data.ema5 = self._ema(self.closes, 5)
            self.data.ema20 = self._ema(self.closes, 20)
            self.data.rsi14 = self._rsi(self.closes, 14)
            
        if self.data.day_open and self.data.day_high and self.data.day_low:
            self.data.vwap = (self.data.day_high + self.data.day_low + self.data.spot) / 3
            
        self.data.timestamp = datetime.now()
    
    def _fetch_option_chain(self, spot: float) -> Tuple[Optional[Dict], Optional[str]]:
        try:
            r = self.client.expiry_list(Config.NIFTY_SECURITY_ID, Config.NIFTY_EXCHANGE)
            if r.get("status") != "success":
                return None, None
                
            expiries = r.get("data", {}).get("data", [])
            if not expiries:
                return None, None
                
            expiry = expiries[0]
            r = self.client.option_chain(Config.NIFTY_SECURITY_ID, Config.NIFTY_EXCHANGE, expiry)
            if r.get("status") != "success":
                return None, None
            ...
            # V4: Wider ±15 strikes
            allowed = {atm + i * 50 for i in range(-15, 16)}
            
            # Actual Dhan API format: r["data"]["data"]["oc"] is a dict keyed by strike string
            oc_data = r.get("data", {}).get("data", {}).get("oc", {})
            for strike_str, strike_data in oc_data.items():
                ...
                for opt_type in ['CE', 'PE']:
                    key = opt_type.lower()
                    d = strike_data.get(key, {})
                    if not d:
                        continue
                        
                    ltp = float(d.get("last_price", 0))
                    if not (10 <= ltp <= Config.PREMIUM_MAX):
                        continue
                    ...
                    cont = OptionContract(
                        ...
                        ltp=ltp,
                        iv=float(d.get("implied_volatility", 0)),
                        ...
                        bid=float(d.get("top_bid_price", ltp)),
                        ask=float(d.get("top_ask_price", ltp))
                    )
                    
                    if strike not in chain:
                        chain[strike] = {}
                    chain[strike][opt_type] = cont
                    
            return chain, expiry
            
        except Exception as e:
            log.warning(f"[DATAFEED] Chain error: {e}")
            return None, None
    ...
    def _calculate_pcr(self, chain: Dict):
        call_oi = sum(c['CE'].oi for c in chain.values() if 'CE' in c and hasattr(c['CE'], 'oi'))
        put_oi = sum(c['PE'].oi for c in chain.values() if 'PE' in c and hasattr(c['PE'], 'oi'))
        
        self.data.call_oi_total = call_oi
        self.data.put_oi_total = put_oi
        
        if call_oi > 0:
            self.data.pcr = put_oi / call_oi
        else:
            self.data.pcr = 1.0
            
        bias, count, raw = calc_pcr_bias(self.data.pcr, put_oi, call_oi)
        self.data.pcr_bias = bias
        self.data.pcr_zone_count = count
        self.data.pcr_raw_zone = raw
        ...
        for strike, contracts in chain.items():
        ...
        self.data.max_call_oi_strike = max_call_strike
        self.data.max_put_oi_strike = max_put_strike
